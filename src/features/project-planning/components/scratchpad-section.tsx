    ...
    <section>
      <header className="mb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Notes, prompts, questions and decisions</p>
        </div>
      ...
      {items.length === 0 ? (
        ...
        <ul className="grid gap-2 md:grid-cols-2">
          {items.map((s) => (
            <li key={s.id} className="rounded-lg border border-border bg-sidebar/40 p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-[10px] text-muted-foreground">{s.createdAt}</p>
                </div>
                <AdminOnly isAdmin={isAdmin}>
                  <div className="flex items-center gap-1 shrink-0">
                    <MoveMenu from="scratch" onMove={(to) => onMove(s.id, to)} />
                    <button
                      aria-label="Edit"
                      onClick={() => onEdit(s.id)}
