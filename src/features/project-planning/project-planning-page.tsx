...
} from "./components/edit-section-dialog";
import {
...
const closedNice: NiceEditState = { open: false, title: "", initial: emptyNice, onSubmit: () => {} };
...
export function ProjectPlanningPage({
  ...
  const [customSections, setCustomSections] =
    useState<CustomSection[]>(initialCustomSections);
  ...
  const [sectionItemEdit, setSectionItemEdit] =
    useState<SectionItemEditState>(closedSectionItem);
  ...
  function handleEditFeature(id: string) {
    ...
    setEdit({
      ...
      onSubmit: (next) => {
        setFeatures((prev) =>
          prev.map((f) => (f.id === id ? { ...f, title: next } : f)),
        );
    ...
    >
      <div className="mx-auto max-w-5xl px-6 py-6 space-y-8">
        <header className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
              A public view of what we&apos;re exploring, planning, building, and have shipped.
        ...
        {error && (
          <div
            role="alert"
            className="flex items-start justify-between gap-3 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-300"
          >
            <span>{error}</span>
            <button
        ...
        <section className="space-y-3">
          ...
          {visibleFeatures.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              {isAdmin ? "Nothing here yet." : "Nothing on the roadmap yet — check back soon."}
            </p>
          ) : selected || view === "list" ? (
      ...
      <EditSectionItemDialog
        ...
        onOpenChange={(open) =>
          setSectionItemEdit((prev) => ({ ...prev, open }))
        }
      />
    </PlanningShell>
  );
