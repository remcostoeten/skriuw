...
export function useCreateJournalEntry() {
  const mutation = useApiMutation<CreateJournalEntryInput, JournalEntry, JournalEntry[]>(
    createJournalEntry,
    {
  ...
  return {
    ...mutation,
    mutate: (input: CreateJournalEntryInput, options?: Parameters<typeof mutation.mutate>[1]) => {
      const entryId = input.id ?? crypto.randomUUID();
      mutation.mutate(input, {
        ...options,
        onSuccess: (...args) => {
          trackJournalEntryCreated(entryId, { dateKey: input.dateKey, mood: input.mood });
...
export function useUpdateJournalEntry() {
  const mutation = useApiMutation<UpdateJournalEntryInput, JournalEntry | undefined, JournalEntry[]>(
    updateJournalEntry,
    {
      invalidateKeys: [journalKeys.entries()],
      optimistic: {
      ...
      mutation.mutate(input, {
        ...
        onSuccess: (...args) => {
          if (input.mood !== undefined) {
            trackMoodLogged(input.id, String(input.mood));
          } else {
            trackJournalEntryUpdated(input.id);
          }
          options?.onSuccess?.(...args);
        },
      });
...
export function useDeleteJournalEntry() {
  const mutation = useApiMutation<string, void, JournalEntry[]>(deleteJournalEntry, {
    invalidateKeys: [journalKeys.entries()],
    optimistic: {
      queryKey: journalKeys.entries(),
  ...
  return {
    ...
    mutate: (input: string, options?: Parameters<typeof mutation.mutate>[1]) => {
      mutation.mutate(input, {
        ...options,
        onSuccess: (...args) => {
          trackJournalEntryDeleted(input);
          options?.onSuccess?.(...args);
        },
      });
    },
...
export function useCreateJournalTag() {
  const mutation = useApiMutation<CreateJournalTagInput, JournalTag, JournalTag[]>(
    createJournalTag,
    {
  ...
  return {
    ...mutation,
    mutate: (input: CreateJournalTagInput, options?: Parameters<typeof mutation.mutate>[1]) => {
      mutation.mutate(input, {
        ...options,
        onSuccess: (...args) => {
          trackTagCreated(input.name, { color: input.color });
          options?.onSuccess?.(...args);
        },
      });
    },
...
export function useDeleteJournalTag() {
  const mutation = useApiMutation<string, void, JournalTag[]>(deleteJournalTag, {
