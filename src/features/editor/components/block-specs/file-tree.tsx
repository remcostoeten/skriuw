...
type FileTreeEditor = {
  ...
  updateBlock: (
    block: unknown,
    update: { type: "fileTree"; props: { source: string } },
  ) => void;
};
...
function FileTreeBlockView({
  ...
  const totals = useMemo(
    () => countFileTreeNodes(parsedTree.children),
    [parsedTree.children],
  );
    ...
    >
      <header className="flex h-9 items-center gap-2 border-b border-border/40 bg-muted/30 pl-3 pr-1.5">
        ...
        >
          ...
          >
            {copied ? (
              <Check className="h-3.5 w-3.5 text-emerald-400" strokeWidth={1.8} />
            ) : (
      ...
      {editing ? (
        ...
        <div
          role="tree"
          aria-label={parsedTree.rootName}
          className="py-1.5"
        >
