...
import {
  ...
  type PropertyType,
} from "@/features/notes/components/document-properties";
...
function isJournalEntry(content: string): boolean {
  ...
  return (
    /^#\s+\w+\s+\d+,\s+\d{4}/m.test(content) || (/mood:/i.test(content) && /tags:/i.test(content))
  );
}
...
export function Editor({ file, editorMode, isMobile = false, onContentChange }: EditorProps) {
  ...
  const isJournal = useMemo(() => (file ? isJournalEntry(file.content) : false), [file?.content]);
  ...
  const documentProperties = useMemo((): DocumentProperty[] => {
    ...
    return [
      {
        id: "tags",
        type: "tags" satisfies PropertyType,
        name: "Tags",
        value: journalMeta.tags,
      },
      {
        id: "mood",
        type: "mood" satisfies PropertyType,
        name: "Mood",
        value: journalMeta.mood,
      },
      {
        id: "date",
        type: "date" satisfies PropertyType,
        name: "Date",
  ...
  const handlePropertyChange = useCallback(
    (propertyId: string, value: unknown) => {
      ...
      if (propertyId === "mood") {
        const updated = updateJournalContent(
          file.content,
          value as MoodLevel | undefined,
          journalMeta.tags,
        );
  ...
  return (
    <div className={cn(containerClass, "flex relative")}>
      ...
      {isJournal && showMetadataPanel && !isMobile && (
