...
import { SectionHeader, Row, SettingsCard, GroupLabel } from "@/features/settings/components/settings-primitives";
...
export function AccountSection() {
  ...
  const avatarSeed = getAvatarSeed(user?.email || user?.name || user?.id, "workspace-user");
  ...
  return (
    <>
      ...
      <SettingsCard>
        <Row title="Sign out everywhere" description="End all active sessions on other devices.">
          <Button variant="outline" size="sm" onClick={handleSignOut} disabled={isSigningOut}>
            <LogOut className="size-3.5" />
            {isSigningOut ? "Signing out…" : "Sign out"}
          </Button>
        </Row>
        <Row title="Delete account" description="Permanently remove your workspace and notes.">
          <Dialog
            open={deleteOpen}
            onOpenChange={(o) => {
              setDeleteOpen(o);
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  This will permanently remove your workspace, notes, and history. This cannot be
                  undone.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-2">
                <Label htmlFor="delete-confirm" className="text-xs text-muted-foreground">
                  To confirm, type{" "}
                  <span className="font-mono text-foreground">{DELETE_PHRASE}</span> below.
                </Label>
                <Input
                  id="delete-confirm"
                  value={deleteValue}
