...
import { getEditorLineHeightValue } from "@/features/editor/lib/editor-line-height";
...
export function JournalEntryEditor({ selectedDate }: JournalEntryEditorProps) {
	...
	const { content, setContent, entry, saveState, handleMoodSelect, handleRemoveTag } =
	...
	const getTagColor = useMemo(
		() =>
			(tagName: string) =>
				tags.find((tag) => tag.name === tagName)?.color ?? "hsl(var(--project-blue))",
		[tags],
	);
					...
					{entryTags.map((tagName) => (
						...
						>
							...
							<button
								...
								className="rounded-full border border-transparent p-0.5 transition-colors hover:border-current/20 hover:bg-foreground/10"
							>
