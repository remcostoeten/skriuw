...
import type { Folder, Item, Note } from "@/features/notes/types";
...
export async function getNoteTree(): Promise<Item[]> {
        const db = getDb();
...
export async function createFolderRecord(data: {
        name: string;
...
export async function renameItemRecord(id: string, newName: string): Promise<Item | undefined> {
        const db = getDb();
        const noteRow = await db.query.notes.findFirst({ where: eq(notes.id, id) });
        if (noteRow) {
                const updatedAt = now();
...
export async function moveItemRecord(id: string, targetFolderId: string | null): Promise<boolean> {
        const db = getDb();
        const noteRow = await db.query.notes.findFirst({ where: eq(notes.id, id) });
        if (noteRow) {
                await db
...
export async function deleteItemRecord(id: string): Promise<boolean> {
        const db = getDb();
...
export async function readNoteEntities(options?: {
        ...
        if (options?.getById) {
                return getItemById(options.getById);
        }

        const items = await getNoteTree();
