    ...
    return (
      <div key={folder.id}>
        <button
          ...
          className="w-full flex items-center gap-1.5 px-3 py-1.5 text-sm text-foreground/70 hover:bg-theme-hover transition-colors"
          style={{ paddingLeft: `${12 + depth * 16}px` }}
        >
          <ChevronRight
            className={cn(
              'w-3.5 h-3.5 transition-transform text-theme-dim',
              folder.isOpen && 'rotate-90'
            )}
          />
          <Folder className="w-4 h-4 text-theme-dim" strokeWidth={1.5} />
          <span className="flex-1 text-left truncate">{folder.name}</span>
          <span className="text-xs text-theme-dim">{childCount}</span>
        </button>
        {folder.isOpen && (
          <div>
    ...
    <button
      ...
      className={cn(
        ...
        activeFileId === file.id
          ...
          : 'text-foreground/70 hover:bg-theme-hover'
      )}
      style={{ paddingLeft: `${12 + depth * 16}px` }}
    >
      {file.name}
