...
export function MarkdownRenderer({ content }: MarkdownRendererProps) {
  const rendered = useMemo(() => {
      ...
      while (remaining.length > 0) {
        ...
        switch (first.type) {
          ...
          case 'link':
            ...
            continue;
          case 'code':
            parts.push(<code key={key++} className="px-1.5 py-0.5 rounded bg-theme-hover text-sm font-mono">{first.match[1]}</code>);
            remaining = remaining.slice(first.index + first.match[0].length);
            continue;
        }
        break;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^---+$/.test(line.trim())) {
        flushList();
        elements.push(<hr key={i} className="border-theme-divider my-6" />);
        continue;
      }
