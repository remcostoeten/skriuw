    ...
    return (
        <aside className="w-48 transition-all border-r duration-300 overflow-hidden bg-background/50 backdrop-blur-sm">
            <div className="relative top-0 flex flex-col min-h-10 w-full border-b bg-background overflow-hidden">
                ...
                />
                <div className={`absolute pb-[0.5px] flex flex-row items-center justify-center w-full h-full px-[5px] gap-1 shrink-0 transition-all duration-300 ${searchState.isOpen ? 'pointer-events-auto translate-y-0' : 'pointer-events-none translate-y-10'}`}>
                    <SidebarSearch
            ...
            <div className="overflow-y-auto h-[calc(100vh-80px)] px-2">
                {searchState.query && (
                    <div className="mb-2 p-2 text-xs text-muted-foreground bg-muted/50 rounded">
                        ...
                        {searchState.options.caseSensitive && ' (Case Sensitive)'}
                        {searchState.options.wholeWord && ' (Whole Word)'}
                    </div>
                )}

                {searchState.query ? (
                    <>
                        {folders.map(function ({ item: folder, highlightedText }) {
                            const noteCount = notes.filter(function (n) {
                                return (n.item as any).folder?.id === folder.id;
                            }).length;
                        ...
                        {notes.map(function ({ item: note, highlightedText }) {
                            return (
