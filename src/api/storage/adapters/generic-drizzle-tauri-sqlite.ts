...
export function createGenericDrizzleTauriSqliteAdapter(options: TauriSqliteOptions = {}): GenericStorageAdapter {
        ...
        async function saveEntities<T extends BaseEntity>(storageKey: string, entities: T[]): Promise<void> {
                ...
                await proxy
                        ...
                        .onConflictDoUpdate({ target: storageRecords.storageKey, set: { payload, updatedAt: timestamp } })
        }
        ...
        const adapter: GenericStorageAdapter = {
                ...
                async getStorageInfo(): Promise<StorageInfo> {
                        const rows = await proxy.select().from(storageRecords)
                        let totalItems = 0
                        let totalSize = 0
