...
export const DEFAULT_FILE_TREE_SOURCE = `Skriuw starter workspace
...
export function parseFileTreeSource(source: string): ParsedFileTree {
  ...
  const rootName =
    firstTreeLineIndex > 0
      ? lines.slice(0, firstTreeLineIndex).join(" ").trim()
      : "File tree";
  const treeLines = toParsedLines(firstTreeLineIndex >= 0 ? lines.slice(firstTreeLineIndex) : lines);
