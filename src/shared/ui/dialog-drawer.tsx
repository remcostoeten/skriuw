        ...
        <Portal>
          <div className="fixed inset-0 z-[9999] flex items-center justify-center">
            <motion.div
              ...
              className={`w-[90%] h-[90%] top-[5%] right-[5%] bottom-[5%] left-[5%] fixed bg-popover text-popover-foreground rounded-lg shadow-lg border border-border p-6 gap-4 flex flex-col ${className}`}
              onClick={handleContentClick}
            >
              {children}
            </motion.div>
  ...
  return (
    <div className={className || "flex flex-col items-start gap-2 w-full"}>
      {label && (
        <label
          className={
            labelClassName || "font-medium text-muted-foreground text-xs pl-2"
          }
        >
          {label}
        </label>
...
export function DialogContentArea({
  ...
  return (
    <div
      className={
        className ||
        "flex flex-col items-start justify-start gap-2 h-full flex-1 text-foreground"
      }
    >
      {children}
    </div>
