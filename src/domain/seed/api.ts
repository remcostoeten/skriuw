...
export async function ensureCloudStarterContentSeeded(userId: string): Promise<void> {
	...
	const [{ data: existingNotes }, { data: existingFolders }] = await Promise.all([
		supabase.from("notes").select("id").eq("user_id", userId).is("deleted_at", null).limit(1),
		supabase.from("folders").select("id").eq("user_id", userId).is("deleted_at", null).limit(1),
	]);
