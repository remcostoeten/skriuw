...
export function buildNoteVersionContentHash(
  candidate: NoteVersionCandidate,
): string {
...
export function shouldPersistNoteVersion(
  ...
  const elapsed =
    candidate.createdAt.getTime() - latestVersion.createdAt.getTime();
  if (elapsed >= NOTE_VERSION_MIN_INTERVAL_MS) {
    return true;
  }

  const charDelta = Math.abs(
    countChars(candidate.content) - countChars(latestVersion.content),
  );
  ...
  const wordDelta = Math.abs(
    countWords(candidate.content) - countWords(latestVersion.content),
  );
