...
import type { TokenName } from "../../../../shared/theme/tokens";
...
export type Mood = {
  ...
  label: string;
  icon: string;
  /** Generated theme token the bar and the glyph take their colour from (R-A5). */
  token: TokenName;
};
...
/**
 * The same five levels, labels and glyphs the desktop journal offers
 * (`app/src/features/journal/model.ts`). Only the colour differs in kind: the
 * desktop carries a Tailwind class and the shell carries a generated token,
 * because React Native has no stylesheet to name.
 */
export const MOOD_OPTIONS: Record<MoodLevel, Mood> = {
  great: { level: "great", label: "Great", icon: "++", token: "mood-great" },
  good: { level: "good", label: "Good", icon: "+", token: "mood-good" },
  neutral: { level: "neutral", label: "Neutral", icon: "~", token: "mood-neutral" },
  low: { level: "low", label: "Low", icon: "-", token: "mood-low" },
  rough: { level: "rough", label: "Rough", icon: "--", token: "mood-rough" },
};
 ...
 * reused while the maps it reads keep their identity, so the calendar, the
 * trend strip and the section share one pass and unrelated store updates cost
 * a handful of comparisons. Days opened but never written to (no words, no
 * mood) stay out, so browsing the calendar never fabricates visible entries.
 */
