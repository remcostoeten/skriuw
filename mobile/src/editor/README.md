...
| `editor-host.tsx` | The React Native host: mounts the surface once, keyboard avoidance, safe areas, the recoverable surface. |
...
Three things live outside this directory and are not done yet:

1. `mobile/package.json` needs `react-native-webview` and `@expo/dom-webview`.
   Without them Metro cannot bundle a `'use dom'` component.
2. The built editor page must be packaged and served from the DOM component's
   origin, and `EXPO_PUBLIC_SKRIUW_EDITOR_ENTRY` must point at it. Until then
   the host shows the `bundle-missing` surface instead of a blank webview.
3. `mobile/src/shell/shell-frame.tsx` must mount `<EditorHost visible={route === "notes"} />`
   once, beside the route column rather than inside it, and `route-views.tsx`
   must stop subscribing to `markdown` — that subscription re-renders the shell
   on every keystroke. Android also needs `softwareKeyboardLayoutMode: "resize"`
   in `app.json`.

## Checks

`scripts/check-mobile.sh` typechecks this directory. The tests here are
`.cts` and, like `src/bridge` and `src/shell`, are outside that gate; run them
with `bunx tsx --test src/editor/__tests__/*.test.cts` from `mobile/`.
