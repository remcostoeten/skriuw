...
import { SHELL_DESTINATIONS, type ShellIconName } from "../destinations";
...
test("notes is the open folder on mobile too", () => {
  assert.equal(ICON_REGISTRY.notes.glyph, "folder_open");
  const notes = SHELL_DESTINATIONS.find((destination) => destination.route === "notes");
  assert.equal(notes?.icon, "notes");
});
...
test("frames are SVG transforms react-native-svg can parse", () => {
  ...
  assert.match(frame.transform, /^translate\([-\d. ]+\) rotate\([-\d.]+\) scale\([-\d. ]+\) skewX\([-\d.]+\) translate\([-\d. ]+\)$/);
});
