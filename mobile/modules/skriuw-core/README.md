...
Expo autolinks everything under `mobile/modules/`, so the application needs no
...
adapter (`mobile/src/bridge`) owns parsing them into the generated contract
...
mobile/modules/skriuw-core/scripts/build-android.sh        # release; pass dev for a debug core
```
...
```bash
mobile/modules/skriuw-core/scripts/build-android.sh
cd mobile && bunx expo run:android
```

`expo run:android` prebuilds `mobile/android` (gitignored), compiles the native
...
to come back from native SQLite. The probe route is copied into `mobile/app`
