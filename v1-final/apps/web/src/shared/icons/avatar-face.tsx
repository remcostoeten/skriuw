...
export function AvatarFace({ name, size = 36, color, className }: AvatarFaceProps) {
	...
	return (
		<div
			...
			aria-hidden="true"
		>
			<div className="absolute inset-0 bg-[radial-gradient(circle_at_center,hsl(var(--foreground)/0.16),transparent_62%)]" />
			<div
			...
			>
				...
				>
					{shape.paths.map((path, index) => (
						<path key={index} d={path} fill="hsl(var(--avatar-ink))" />
					))}
				</svg>
				<span
					className="absolute left-1/2 top-full mt-[8%] -translate-x-1/2 font-mono font-bold leading-none"
					style={{ color: "hsl(var(--avatar-ink))", fontSize }}
				>
					{data.initial}
				</span>
			</div>
