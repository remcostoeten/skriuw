...
function render(name: (typeof ICON_NAMES)[number], animated: boolean): string {
  return renderToStaticMarkup(
    createElement(AnimatedIconsProvider, { enabled: animated }, createElement(AppIcon, { name, size: 18 })),
  );
}
...
test("every action icon draws its 24-unit Fluent glyph at rest", () => {
  for (const name of ICON_NAMES) {
    ...
    assert.ok(!html.includes("data-motion-parts"), `${name}: parts rendered while animation is off`);
  }
});
