...
export const SECTIONS = [
  ...
  },
  {
    ...
    searchText:
      "artificial intelligence providers models writing tools prompts prompt library built-in custom system prompt rewrite improve grammar shorten lengthen tone simplify translate summarize title outline continue duplicate reset modified default model switcher ollama gemini groq local remote playground prompt test stream usage history runs tokens token count cost spend estimate estimated retention prune clear history metadata only",
    icon: StarIcon,
    desktopOnly: true,
  },
  {
