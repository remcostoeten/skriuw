...
export function useAiRun(signal: AbortSignal, { origin, onStart }: Options): AiRunController {
   ...
   * until its terminal has been delivered, so a replayed effect that re-sent
   * the caller's id would be refused as a duplicate while the first send was
   * still in flight — and the first send's consumer is already gone.
   */
  const fire = useCallback(function fire(template: AiCompletionRequest): void {
