...
type AiRunStepId = "send" | "answer" | "write";
...
/** The one colour the card wears at a time, so its accent never contradicts its message. */
export type AiRunTone = "idle" | "live" | "result" | "failed" | "stopped";

export function aiRunTone(run: AiActionRun): AiRunTone {
  if (run.phase === "streaming") {
    return "live";
  }
  if (run.phase === "done") {
    return run.preview.trim().length > 0 ? "result" : "idle";
  }
  if (run.phase === "cancelled") {
    return "stopped";
  }
  if (run.phase === "error" || run.phase === "timeout") {
    return "failed";
  }
  return "idle";
}
