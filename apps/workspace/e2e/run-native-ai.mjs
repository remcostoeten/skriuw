...
const output = resolve(
  root,
  outputIndex >= 0
    ? (process.argv[outputIndex + 1] ?? "apps/workspace/e2e/results/native-ai-latest.json")
    : "apps/workspace/e2e/results/native-ai-latest.json",
);
...
try {
  ...
  const result = {
    ...
    command: `node apps/workspace/e2e/run-native-ai.mjs${skipBuild ? " --skip-build" : ""} --output ${output}`,
    machine: {
      hostname: hostname(),
      platform: platform(),
      release: release(),
