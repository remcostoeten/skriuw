...
try {
  ...
  await expectTitles(
    cdp,
    sessionId,
    { present: [FIRST_NOTE], absent: [SECOND_NOTE] },
    "before any sign-in",
  );

  const claimed = await evaluate(cdp, sessionId, adoptExpression(FIRST_ACCOUNT));
  if (claimed !== "claimed")
    throw new Error(`first sign-in did not claim the workspace: ${claimed}`);
  ...
  await expectTitles(
    cdp,
    sessionId,
    { present: [FIRST_NOTE], absent: [] },
    "after the first account claimed it in place",
  );

  const active = await evaluate(cdp, sessionId, adoptExpression(FIRST_ACCOUNT));
  if (active !== "active")
    throw new Error(`signing into the owner again was not a no-op: ${active}`);

  await switchAccount(cdp, sessionId, SECOND_ACCOUNT, "second account");
  await expectTitles(
    cdp,
    sessionId,
    { present: [], absent: [FIRST_NOTE] },
    "on the second account's fresh workspace",
  );
  ...
  await expectTitles(
    ...
    "after writing on the second account",
  );
  ...
  for (const expected of [
    ...
    if (!panelText.includes(expected)) {
      throw new Error(
        `account settings do not name the owning workspace (${expected}): ${JSON.stringify(panelText)}`,
      );
    }
  }
...
async function openAccountSettings(cdp, sessionId) {
  ...
  await waitFor(
    cdp,
    sessionId,
    `Boolean(document.getElementById("settings-tab-account"))`,
    "settings dialog",
  );
