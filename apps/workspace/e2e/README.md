...
```bash
node apps/workspace/e2e/run.mjs [--provider-import-only | --tasks-only | --personal-only | --journal-only | --mermaid-only]
```
...
network. `apps/workspace/performance` uses the same two seams.
...
```bash
node apps/workspace/e2e/run-native.mjs [--skip-build] [--output <path>]
# or, from apps/workspace/: bun run e2e:native
```
...
```bash
node apps/workspace/e2e/run.mjs --journal-only --output /tmp/skriuw-journal-e2e.json
```
...
```bash
node apps/workspace/e2e/run.mjs --personal-only --output /tmp/skriuw-personal-e2e.json
```
...
```bash
node apps/workspace/e2e/mobile-shell.mjs [--output <path>]
# or, from apps/workspace/: bun run e2e:mobile
```
...
```bash
node apps/workspace/e2e/browser-account-switch.mjs
# or, from apps/workspace/: bun run e2e:browser-account-switch
```

Previews on port 4196 (`SKRIUW_E2E_ACCOUNT_PORT` overrides it) and runs as
part of `bin/check browser`.
