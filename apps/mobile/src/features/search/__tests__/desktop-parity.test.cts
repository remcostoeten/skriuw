...
import { parseSearchQuery as parseOnDesktop } from "../../../../../workspace/src/features/search/query-parser";
import { parseSearchQuery } from "../query/query-parser";

const featureDir = resolve(__dirname, "..");
const desktopDir = resolve(__dirname, "../../../../../workspace/src/features/search");
...
for (const module of COPIED_MODULES) {
  test(`${module} has not drifted from the desktop original`, () => {
    ...
    assert.equal(
      mobile,
      desktop,
      `${module} differs from apps/workspace/src/features/search/${basename(module)}; copy the change across or extract both into shared/renderer-core.`,
    );
  });
}
