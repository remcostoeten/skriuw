/**
 * The forgiving date grammar behind the journal's "Go to date…" sheet. Pure
 * calendar math over `DateKey` strings with no clock of its own; the grammar is
 * specified in `docs/specs/journal-navigation.md`.
 *
 * The desktop twin is `apps/workspace/src/features/journal/date-expressions.ts`. Both
 * runtimes answer the same expressions, which `__tests__/parity.test.cts`
 * holds them to.
 */
