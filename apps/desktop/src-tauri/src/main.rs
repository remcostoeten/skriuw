...
fn main() {
    #[cfg(target_os = "linux")]
    {
            ...
            && std::env::var("GDK_BACKEND").as_deref() == Ok("x11")
        {
            ...
            // AppImage's AppRun (tauri-apps/tauri#8541), which drags us through
            // XWayland. WebKit's dmabuf renderer cannot allocate GBM buffers
            // there on NVIDIA, so the window paints black. The .deb and dev
            // builds already run the Wayland backend natively on this stack;
            // match them. SKRIUW_GDK_BACKEND opts back out.
            std::env::set_var("GDK_BACKEND", "wayland");
        }

        // Same GBM allocation failure ("Failed to create GBM buffer: Invalid
        // argument" -> black window) hits any genuine X11 session on NVIDIA.
        // The shared-memory path is slower but is the only one that renders
        // there; Wayland/NVIDIA and every Mesa driver keep the GPU path.
        if nvidia
