...
pub use installer::{stop_managed_server, OllamaInstallEvent, OllamaInstallStatus, DEFAULT_ENDPOINT};
...
fn load_config(app: &AppHandle) -> Result<AiConfig, String> {
	...
	Ok(AiConfig {
		...
		groq_model: ai_str(&settings, "groqModel").unwrap_or_else(|| DEFAULT_GROQ_MODEL.to_string()),
...
fn prompt_for(action: &str, content: &str) -> Result<(String, String), String> {
	let (system, user) = match action {
		...
		"continueWriting" => (
			"You are a writing assistant that continues documents in the author's voice.",
			format!(
...
#[tauri::command]
pub async fn ai_complete(app: AppHandle, action: String, content: String) -> Result<String, String> {
...
pub async fn ai_install_ollama(
	...
	let outcome = installer::install_managed(root, config.ollama_endpoint, on_event.clone(), cancel).await;
