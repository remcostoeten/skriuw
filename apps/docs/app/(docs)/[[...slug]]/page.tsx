  ...
  return (
    <DocsPage toc={document.data.toc}>
      ...
      <DocsBody>
        ...
        {isSectionIndex(page.path) && <SectionPages url={page.url} />}
      </DocsBody>
    </DocsPage>
  );
}
...
function isSectionIndex(filePath: string) {
  const segments = filePath.split("/");
  return segments.length > 2 && /^index\.mdx?$/.test(segments.at(-1) ?? "");
}

function SectionPages({ url }: SectionPagesProps) {
  return (
    <Cards>
      {getPageTreePeers(source.getPageTree(), url).map((peer) => (
        <Card key={peer.url} href={peer.url} title={peer.name}>
          {peer.description}
        </Card>
      ))}
    </Cards>
