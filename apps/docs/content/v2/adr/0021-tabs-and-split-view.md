---
title: "ADR-0021: Tabs and split view"
---
...
The product ships with one active note and one persistent ProseMirror editor host by deliberate decision (ADR-0020): "one persistent direct ProseMirror view" and "the application shell and persistent editor host hold no workspace subscription" are measured, load-bearing performance properties. The bounded-editor acceptance gate ([Product bounded-editor fallback](/v2/benchmarks/2026-07-22-product-bounded-editor)) proves one editor host and one editor instance survive 100 note switches.

Tabs and split view ([Tabs and split view](/v2/specs/tabs-and-split-view)) require more than one note to be *open* at once, and — for split view only — more than one note to be *live* at once. This changes a documented architectural invariant, so the spec requires an accepted ADR before implementation.
