---
title: "Workspace operation sync policy v1"
---
...
[`workspace-operation-sync-policy-v1.json`](https://github.com/remcostoeten/skriuw/blob/daddy/contracts/generated/workspace-operation-sync-policy-v1.json)
...
| `SetNoteCoverGradient` | Replicated | Requires the note; the gradient names a built-in id, so it carries no asset dependency. Concurrent cover selections resolve by server order later. | Must not resurrect a purged note; setting a gradient clears any image cover and follows the same media-retention rules. | Small metadata; inline. | Missing/purged note and an unknown gradient id are explicit conflicts. |
...
[the local sync outbox contract](/v2/specs/local-sync-outbox). This policy does not
implement remote application, chunks, tombstone storage, or convergence. Those
remain separate tasks in the [cloud sync master tracker](/v2/specs/cloud-sync-master).
