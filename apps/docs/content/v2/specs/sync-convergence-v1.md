...
[ADR-0037](/v2/adr/0037-automatic-sync-convergence) records the decision.
...
- [ADR-0026](/v2/adr/0026-optional-cloud-operation-replication) defines the
  local-first replication architecture and
  [ADR-0037](/v2/adr/0037-automatic-sync-convergence) the convergence
  decision.
- [Workspace operation sync policy v1](/v2/specs/workspace-operation-sync-policy-v1)
  defines admission and transport classes.
- [Local sync outbox](/v2/specs/local-sync-outbox) defines local commit, enqueue,
  retry, echo, blocked-row, and received-record behavior.
- [Desktop sync coordinator](/v2/specs/desktop-sync-coordinator) defines triggers,
  status projection, and truncation recovery.
- [Data model](/v2/data-model), [subtree trash and purge](/v2/adr/0009-subtree-trash-and-purge),
  and [portable archives](/v2/adr/0007-portable-workspace-archive) define
  current canonical local behavior.
- [Recovery](/v2/recovery) defines verified native backup and restore.
...
| `SetNoteCoverGradient` / replicated | Note; a gradient id this build knows | Later server sequence wins; the same gradient is **No-op**; a non-null gradient clears the image cover in the same step | Different notes commute | Purge blocks without resurrection | Apply/no-op or `R(domain_conflict)` / `R(tombstone_blocked)` | gradient versus image cover; gradient/gradient; unknown id from a newer build; purged note |
...
[sync content operations](/v2/specs/sync-content-operations): an operation is
...
- Device-local settings, active note state, and provider receipts never enter
  ...
  asset transport in
  [content-addressed chunk transport v1](/v2/specs/sync-content-chunks-v1).
