---
title: "Desktop background sync coordinator"
---

Status: implemented; production sync runs against the deployed
[authentication boundary](/v2/specs/cloud-sync-authentication) after an explicit
Account-settings connection.

`crates/skriuw-sync` owns the background lifecycle between the durable
[native sync queue](/v2/specs/local-sync-outbox) and the workspace sync service.
The same cycle runs inside the browser storage worker
([ADR-0028](/v2/adr/0028-browser-worker-owned-sync)); the differences below
are scheduling, not protocol. Scope and completion tracking live in the
[cloud sync master tracker](/v2/specs/cloud-sync-master).
...
[sync convergence v1](/v2/specs/sync-convergence-v1); an operation that does not
...
transport](/v2/specs/sync-content-chunks-v1) in three places, all off the interaction
...
- **Rehydration after log truncation.** When pull answers `log_truncated`,
  ...
  [sync convergence v1 §5](/v2/specs/sync-convergence-v1#hydration-and-rehydration).
...
[sync convergence v1 §6](/v2/specs/sync-convergence-v1#6-executable-scenario-catalogue)
...
latency is measured under [Benchmarks](/v2/benchmarks).
