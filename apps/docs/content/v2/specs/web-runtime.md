...
[Browser runtime scale and cross-browser durability: 2026-08-05](/v2/benchmarks/2026-08-05-browser-runtime-scale)) are captured. Safari
...
- `./bin/check browser` builds `skriuw-domain` and `skriuw-storage` for that
...
- Migrations: the same SQL files under `migrations/` must apply unmodified inside the worker ([Data model](/v2/data-model)'s "migration execution remains adapter-owned so a future SQLite-WASM implementation can apply the same SQL files inside its worker" is an explicit design commitment already made — honor it; do not fork the migration files).
...
`apps/workspace/e2e/browser-storage.mjs` runs the real application bridge in headless
...
- Run the existing archive fixtures ([Archive compatibility fixtures](/v2/archive-fixtures), ADR-0019) through the wasm adapter's import/export path and assert byte-identical or semantically-identical round trips to the native path.
- Run the existing tree fixtures (1,000- and 5,000-node scale fixtures per [Scale fixtures](/v2/fixtures)) through the wasm adapter and confirm the renderer-side tree/virtualization code (which is adapter-agnostic already, per the system shape diagram) performs correctly against it — this is mostly a proof that nothing renderer-side accidentally assumed a desktop-only data shape.
...
- The renderer, sidebar, and editor code require zero adapter-specific branching (verified by the fact that `apps/workspace/src/store/**` and above don't import anything from `skriuw-sqlite-wasm` or Tauri-specific bridge code directly).
