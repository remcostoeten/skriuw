...
`pinned_at` is workspace content, not local UI state — unlike sidebar expansion ([Decision records](/v2/adr), N4), it must be included in portable archive export/import and travel with the workspace. Add it to the `WorkspaceArchive` JSON shape and to [Archive compatibility fixtures](/v2/archive-fixtures)'s golden fixtures so the archive-compatibility tests catch any accidental omission. Bump the archive schema version per the existing versioning convention (see ADR-0007 and ADR-0019) since this is a new field older archives won't have; missing `pinned_at` on import must default to `null`, not fail validation.
