---
title: "Cloud sync master tracker"
---
...
conditions, see the companion [cloud and web delivery plan](/v2/specs/cloud-sync-delivery).
...
See [ADR-0026](/v2/adr/0026-optional-cloud-operation-replication).
...
- Cached navigation preserves every budget in
  [the performance contract](/v2/performance-contract).
...
- [x] Preserve the portable `skriuw-domain` and `skriuw-storage`
  [WASM gate](https://github.com/remcostoeten/skriuw/blob/daddy/bin/check).
- [x] Confirm [native SQLite remains canonical](/v2/adr/0002-sqlite-canonical)
  and local-only by default.
- [x] Commit versioned
  [Rust sync contracts](https://github.com/remcostoeten/skriuw/blob/daddy/crates/skriuw-domain/src/sync.rs) and generated
  JSON Schemas.
- [x] Add a
  [cross-language golden wire fixture](https://github.com/remcostoeten/skriuw/blob/daddy/contracts/fixtures/sync-push-v1.json).
- [x] Classify every `WorkspaceOperation` through the exhaustive
  [sync policy](/v2/specs/workspace-operation-sync-policy-v1), including protocol-v1
  unsupported operations and Worker rejection behavior.
- [x] Add an optional
  [local connection record and transactional sync outbox](/v2/specs/local-sync-outbox).
...
- [x] Scaffold the
  [v2-only Worker and SQLite-backed Workspace Durable Object](https://github.com/remcostoeten/skriuw/blob/daddy/services/sync/src/workspace-sync-object.ts).
...
- [x] Test retries, sequence gaps, conflicting duplicates, and
  [workspace isolation in the Workers runtime](https://github.com/remcostoeten/skriuw/blob/daddy/__tests__/services/sync/workspace-sync-object.test.ts).
- [x] Add and test the fail-closed
  [provider-independent authentication and authorization boundary](/v2/specs/cloud-sync-authentication),
...
- [x] Add per-device acknowledgement cursors and bounded
  [log compaction](/v2/specs/sync-content-operations).
- [x] Add R2 content-addressed chunks for large documents through the
  [versioned chunk contract, protocol-v2 admission, and authorized transfer](/v2/specs/sync-content-chunks-v1).
...
- [x] Externalize oversized operations from the client transport end to end,
  with [chunk upload on push and verified reassembly on pull](/v2/specs/sync-content-chunks-v1).
- [x] Publish the compaction floor and answer a pull below it with
  ...
  [sync content operations](/v2/specs/sync-content-operations).
...
- [ ] Add structured metrics and abuse limits beyond the quota. The
  [operational contract](/v2/specs/sync-content-operations) is written; request-rate
...
- [x] Push the durable outbox immediately after local commit through the
  [background sync coordinator](/v2/specs/desktop-sync-coordinator).
...
- [x] Implement SQLite WASM over OPFS using the shared migrations and prove a
  [real Chromium close/reload round trip](https://github.com/remcostoeten/skriuw/blob/daddy/apps/workspace/e2e/browser-storage.mjs).
- [x] Pass native/browser operation and archive fixture parity.
- [x] Define browser history and recovery behavior in
  [ADR-0027](/v2/adr/0027-browser-sqlite-opfs-sah-pool).
...
- [x] Define the [merge behavior per operation family](/v2/specs/sync-convergence-v1).
- [x] Converge automatically with no user-facing conflict: documents by the
  four-rule server-sequence decision in the
  [pure reconciliation decision](https://github.com/remcostoeten/skriuw/blob/daddy/crates/skriuw-domain/src/reconcile.rs),
  every other family last-writer-by-server-sequence, losing bodies preserved
  as history with provenance `superseded`
  ([ADR-0037](/v2/adr/0037-automatic-sync-convergence)).
- [x] Retain terminal identity tombstones that block resurrection, with
  [delete-versus-edit coverage](https://github.com/remcostoeten/skriuw/blob/daddy/crates/skriuw-sqlite/src/tests.rs).
...
- [x] Encrypt cloud content end to end: a sealed payload form and sealed
  ...
  [ADR-0043](/v2/adr/0043-end-to-end-encrypted-sync) and covered by
  [`encrypted_scenarios.rs`](https://github.com/remcostoeten/skriuw/blob/daddy/crates/skriuw-sync/tests/encrypted_scenarios.rs)
  and [`sealed-operations.test.ts`](https://github.com/remcostoeten/skriuw/blob/daddy/__tests__/services/sync/sealed-operations.test.ts).
...
- [x] Test two offline devices reconnecting in either order, duplicate
  ...
  truncation in
  [`cycle_scenarios.rs`](https://github.com/remcostoeten/skriuw/blob/daddy/crates/skriuw-sync/tests/cycle_scenarios.rs).
...
exhaustive [operation policy](/v2/specs/workspace-operation-sync-policy-v1), committed
...
[security boundary](/v2/specs/cloud-sync-authentication) plus deployed Better Auth
...
[connection and transactional outbox](/v2/specs/local-sync-outbox), inbound cursor,
...
provenance; [sync convergence v1](/v2/specs/sync-convergence-v1) is the merge
contract and [ADR-0037](/v2/adr/0037-automatic-sync-convergence) the
decision. The [desktop background coordinator](/v2/specs/desktop-sync-coordinator)
...
retention](/v2/specs/sync-content-operations) behind a published compaction floor,
