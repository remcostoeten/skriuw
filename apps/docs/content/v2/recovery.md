---
title: "Native recovery runbook"
description: "Scheduled native backups are verified single-file SQLite artifacts. They are separate from portable workspace archives and asynchronous Git history."
---
...
Note images live as content-addressed files in a `blobs/` directory next to the database. Each scheduled SQLite artifact has a same-named `.blobs` sidecar in `recovery/` containing exactly the image files referenced by that database snapshot. Restore copies and validates that sidecar before the database goes live; it never removes newer local blobs, because content hashes make them safe to retain. A legacy backup that references images but has no sidecar fails safely instead of restoring a workspace with broken media.
...
A workspace that uses end-to-end encrypted sync keeps its content key in the database, but only for the device that entered the recovery code. Scheduled, manual, and pre-import backups are written without it, and restoring any backup removes it, so a backup file never carries the key to the encrypted cloud copy. After a restore, sync stops with "This workspace is encrypted and this device has no key"; enter the recovery code in Settings → Account & sync to resume. Local notes restored from the backup are unaffected. Moving a workspace with Settings → Data → Move workspace keeps the key, because the workspace stays on the same device. See [ADR-0043](/v2/adr/0043-end-to-end-encrypted-sync).
