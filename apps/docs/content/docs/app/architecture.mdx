...
graph TB
    ...
    Skriuw --> LocalDB[Local Storage]
    Skriuw --> Cloud[Cloud Sync]
...
- Generate IDs
- Add timestamps
- Handle Skriuw transactions
- Provide consistent API

### 4. Skriuw Schema & Client
...
3. createQueryHook executes query
    ↓
4. Skriuw fetches from local cache
    ↓
...
Skriuw generates types from the schema:
...
Skriuw applies changes locally before server confirmation.
