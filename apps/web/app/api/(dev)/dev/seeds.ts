...
function createBlockHelpers() {
	...
	return { heading, paragraph, bulletItem, numberedItem, codeBlock }
}
...
PostgreSQL`,
		...
		},
		{
			name: 'Project Ideas',
			content: [
				heading(1, '💡 Project Ideas'),
				paragraph('Capture your project ideas and plans here.'),
				paragraph(''),
				heading(2, 'Ideas'),
				bulletItem('Build a personal knowledge management system'),
				bulletItem('Create a productivity dashboard'),
				bulletItem('Develop a habit tracking application'),
				paragraph(''),
				heading(2, 'Next Steps'),
				numberedItem('Prioritize ideas based on impact and effort'),
				numberedItem('Create detailed project plans'),
				numberedItem('Set milestones and deadlines')
			]
		},
		{
			name: 'Meeting Notes',
			content: [
				...
				paragraph('Keep track of important meetings and action items.'),
				paragraph(''),
				heading(2, 'Template'),
				bulletItem('Date and attendees'),
				bulletItem('Agenda items'),
				bulletItem('Discussion points'),
				bulletItem('Action items and owners'),
				bulletItem('Follow-up tasks')
			]
		}
	]
	...
	const sampleFolders = [
		{
			...
			children: ['Keyboard Shortcuts', 'Editor Features']
		},
		{
			name: 'Documentation',
			children: ['Architecture Overview', 'Storage & Backup']
		},
		{ name: 'Personal' },
	...
	return { sampleNotes, sampleFolders }
}

export const { sampleNotes, sampleFolders } = generateSampleData()
