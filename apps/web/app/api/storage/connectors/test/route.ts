...
async function testS3(config: Record<string, string>) {
...
async function testDrive(config: Record<string, string>) {
...
async function runHandshake(type: StorageConnectorType, config: Record<string, string>) {
    switch (type) {
        case 's3':
            return testS3(config)
        case 'dropbox':
            return testDropbox(config)
        case 'google-drive':
            return testDrive(config)
        default:
...
export async function POST(request: NextRequest) {
    try {
        ...
        const validated = validateConnectorConfig(type, rawConfig)
        const result = await runHandshake(type, validated)
