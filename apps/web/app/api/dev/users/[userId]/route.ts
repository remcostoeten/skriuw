...
import { getDatabase, notes, folders, tasks, settings, shortcuts, aiUsage, schema } from '@skriuw/db'
...
export async function POST(request: NextRequest, context: RouteContext) {
    ...
    try {
        ...
        if (action === 'reset') {
            ...
            // Delete folders
            const deletedFolders = await db.delete(folders).where(eq(folders.userId, userId)).returning()
            deletedCounts.folders = deletedFolders.length

            // Delete settings
            const deletedSettings = await db.delete(settings).where(eq(settings.userId, userId)).returning()
            deletedCounts.settings = deletedSettings.length

            // Delete shortcuts
            const deletedShortcuts = await db.delete(shortcuts).where(eq(shortcuts.userId, userId)).returning()
            deletedCounts.shortcuts = deletedShortcuts.length

            // Delete AI usage
            const deletedAiUsage = await db.delete(aiUsage).where(eq(aiUsage.userId, userId)).returning()
            deletedCounts.aiUsage = deletedAiUsage.length
        ...
        return NextResponse.json(
            { error: 'Failed to manage user', details: (error as Error).message },
            { status: 500 }
        )
    }
}
