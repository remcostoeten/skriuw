    ...
    for (const currentUser of allUsers) {
      try {
        ...
        if (existingNote.length > 0) {
          ...
          console.log(`⏭️  Skipped user ${currentUser.email} - already has Identity Guard note`)
          continue
        }
        ...
        results.seededUsers++
        console.log(`✅ Seeded Identity Guard note for user: ${currentUser.email} (${currentUser.isAnonymous ? 'anonymous' : 'authenticated'})`)
