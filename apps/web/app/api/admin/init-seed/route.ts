...
async function seedGuestUser() {
	...
	const templatesF = await drizzleDb
		.select()
		.from(seedTemplateFolders)
		.orderBy(seedTemplateFolders.order)
	const templatesN = await drizzleDb
		.select()
		.from(seedTemplateNotes)
		.orderBy(seedTemplateNotes.order)
