...
export function AutoSignIn() {
    ...
    useEffect(() => {
        if (!AUTH_CLIENT_ENABLED) {
            logger.info('auth', 'Auth client disabled (missing NEXT_PUBLIC_APP_URL)')
            return
        }

        logger.info('auth', 'State:', { session: !!session, isPending, error: !!error })

        // Check if auto-sign-in is disabled for development
        if (DEV_FLAGS.DISABLE_AUTO_SIGNIN) {
            logger.info('auth', 'Auto-sign-in disabled via DEV_FLAGS.DISABLE_AUTO_SIGNIN')
            return
        }
