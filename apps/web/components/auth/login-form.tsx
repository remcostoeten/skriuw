...
const styles = {
	...
	authButtonPrimary: { backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)' },
	authButtonOutline: {
		backgroundColor: 'transparent',
		borderWidth: '1px',
		borderStyle: 'solid',
...
export function LoginForm({
	...
	subtitle = 'Sign in to keep your notes synced and secure',
	onSuccess
}: Props) {
	...
	const handleGithubSignIn = useCallback(async () => {
		...
		try {
			await signIn.social(
				...
				{
					onError: (ctx) => {
						setGeneralError(ctx.error.message || 'Failed to sign in with GitHub.')
						setIsLoading(false)
			...
			setGeneralError(err instanceof Error ? err.message : 'Failed to sign in with GitHub.')
			setIsLoading(false)
			setLoadingAction(null)
		}
	}, [])
	...
	const handleGoogleSignIn = useCallback(async () => {
		...
		try {
			await signIn.social(
				...
				{
					onError: (ctx) => {
						setGeneralError(ctx.error.message || 'Failed to sign in with Google.')
						setIsLoading(false)
						setLoadingAction(null)
					}
				}
			...
			setGeneralError(err instanceof Error ? err.message : 'Failed to sign in with Google.')
			setIsLoading(false)
			setLoadingAction(null)
		}
	}, [])
