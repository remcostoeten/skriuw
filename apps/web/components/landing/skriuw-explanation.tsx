...
export function SkriuwExplanation({ onCreateNote }: Props) {
	...
	return (
		<motion.span variants={containerVariants} initial={false} animate='visible'>
			<div className='flex flex-col items-center gap-6 mb-8'>
				<div className='flex flex-col items-center gap-3'>
					<motion.div variants={itemVariants} className='relative'>
						<div className='absolute inset-0 blur-3xl bg-primary/20 rounded-full scale-150' />
						<BrandLogo size={140} className='mb-4 text-foreground relative z-10' />
					</motion.div>
					<motion.h1
				...
				>
					<p>
						A blazingly fast, privacy-focused note-taking app built for everyone.
						Providing an opt-in system for all features (yes, AI is included) rather
						than the usual opt-out system. The tools are here, you just need to opt-in.
					</p>
				</motion.div>
			</div>
			...
			>
				<motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
					...
					>
						...
						<Kbd className='ml-2 relative bg-muted/80 border-border/50'>⌘P</Kbd>
					</Button>
				</motion.div>
				<motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
					<Button
