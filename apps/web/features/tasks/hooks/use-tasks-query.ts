...
export const tasksKeys = {
    ...
    note: (noteId: string, userId?: string | null) => [...tasksKeys.all, 'note', noteId, userId ?? 'all'] as const,
...
export function useUpdateTaskMutation() {
    ...
    return useMutation({
        ...
        onMutate: async ({ taskId, updates }) => {
            ...
            if (previousList) {
                queryClient.setQueryData<Task[]>(tasksKeys.list(userId),
    ...
    return useQuery({
        ...
        queryFn: async () => {
            if (isGuest(userId)) {
                 ...
                 const task = (result.data as Task[] | undefined)?.find(t => t.noteId === noteId && t.blockId === blockId)
                 return task || null
            }
