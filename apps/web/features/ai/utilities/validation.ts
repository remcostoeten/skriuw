...
export function validateConfigBody(
	body: unknown,
	isPartial = false
): { success: true; data: ConfigBody } | { success: false; error: string } {
		...
		if (bodyObj.model === null || bodyObj.model === undefined) {
			...
			if (provider === 'gemini') {
				if (!GEMINI_MODELS.includes(candidateModel as GeminiModel)) {
					return {
						success: false,
						error: `Invalid model for gemini. Must be one of: ${GEMINI_MODELS.join(', ')}`
					}
				}
				...
				if (!GROK_MODELS.includes(candidateModel as GrokModel)) {
					return {
						success: false,
						error: `Invalid model for grok. Must be one of: ${GROK_MODELS.join(', ')}`
					}
				}
			}
