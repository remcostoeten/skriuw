				...
				switch (demos[index]) {
					case 'switch':
						...
						setActiveTab((current) => {
							...
							const nextIndex =
								(currentTabs.findIndex((t) => t.id === current) + 1) %
								currentTabs.length
					...
					case 'close':
						...
						const timeoutId = setTimeout(() => {
							setTabs((prev) => {
								if (prev.length > 2) {
									const tabIndexToClose = prev.findIndex(
										(t) => t.id !== activeTab
									)
					...
					case 'new':
						...
						const newTabTimeout = setTimeout(() => {
							setTabs((prev) => [
								...prev,
								{ id: 'temp', title: 'New Note.md', isActive: false }
							])
			...
			if (value && tabs.length > 1) {
				...
				if (activeTab === tabId) {
					const newActiveIndex = Math.max(
						0,
						mockTabs.findIndex((t) => t.id === tabId) - 1
					)
					...
					<div className='h-full flex flex-col'>
						...
						<div className='flex border-b border-border bg-background/50 backdrop-blur-sm'>
							{tabs.map((tab, index) => (
								<div
									key={tab.id}
									className={`group relative flex items-center px-3 py-2 text-xs font-medium border-r border-border/50 cursor-pointer transition-all duration-200 ${
										tab.id === activeTab
											? 'bg-accent text-accent-foreground border-b-2 border-b-accent'
											: 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
									} ${
										currentDemo === 'switch' && tab.id === activeTab
											? 'animate-pulse'
											: ''
									} ${
										currentDemo === 'close' && tab.id !== activeTab
											? 'animate-pulse opacity-50'
											: ''
									} ${
										currentDemo === 'new' && tab.id === 'temp'
							...
							{/* New tab button */}
							<div
								className={`px-2 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/50 cursor-pointer transition-all duration-200 border-r border-border/50 ${
									currentDemo === 'new' ? 'animate-pulse bg-muted/30' : ''
								}`}
							>
