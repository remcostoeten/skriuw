...
export function useTagsWithCountQuery() {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<TagWithCount[]> => {
            ...
            const notesResult = await readMany<{ id: string; tags?: string[] }>(STORAGE_KEYS.NOTES, { userId })
...
export function useCreateTagMutation() {
    ...
    return useMutation({
        mutationFn: async (input: CreateTagInput): Promise<Tag> => {
            ...
            const existing = existingResult.success && Array.isArray(existingResult.data) ? existingResult.data : []

            const duplicate = existing.find(t => t.name.toLowerCase() === input.name.toLowerCase())
            if (duplicate) {
                return duplicate
            }
...
export function useUpdateTagMutation() {
    ...
    return useMutation({
        mutationFn: async ({ id, ...input }: UpdateTagInput & { id: string }): Promise<Tag | null> => {
