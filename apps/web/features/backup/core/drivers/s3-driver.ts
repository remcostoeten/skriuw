...
async function s3Fetch(
    ...
    const url = endpoint ? `${endpoint.replace(/\/$/, '')}/${bucket}${path}` : `${proto}//${host}${path}`
    ...
    Object.keys(headers).forEach(key => {
        ...
        if (lowerKey !== 'host' && lowerKey !== 'x-amz-content-sha256' && lowerKey !== 'x-amz-date') {
    ...
    const signature = Array.from(signatureRaw).map(b => b.toString(16).padStart(2, '0')).join('');
