    ...
    return useMutation({
        mutationFn: async ({ id, keys }: { id: ShortcutId, keys: KeyCombo[] }) => {
            ...
            if (existing.success && existing.data) {
                const result = await update<CustomShortcut>(STORAGE_KEYS.SHORTCUTS, id, {
                    keys,
                ...
                const result = await create<CustomShortcut>(STORAGE_KEYS.SHORTCUTS, {
                    id,
                    keys,
                    customizedAt: new Date().toISOString()
                }, { userId })
        ...
        onMutate: async ({ id, keys }) => {
            ...
            const previousShortcuts = queryClient.getQueryData<Record<ShortcutId, KeyCombo[]>>(shortcutsKeys.user(userId))

            queryClient.setQueryData(shortcutsKeys.user(userId), (old: Record<ShortcutId, KeyCombo[]> | undefined) => {
...
export function useResetAllShortcutsMutation() {
    ...
    return useMutation({
        mutationFn: async () => {
            const result = await readMany<CustomShortcut>(STORAGE_KEYS.SHORTCUTS, { userId })
            if (result.success && result.data) {
                await Promise.all(result.data.map(s => destroy(STORAGE_KEYS.SHORTCUTS, s.id, { userId })))
            }
        },
        onMutate: async () => {
            await queryClient.cancelQueries({ queryKey: shortcutsKeys.user(userId) })
