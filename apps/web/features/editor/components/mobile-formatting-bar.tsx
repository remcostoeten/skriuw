...
export function MobileFormattingBar({ editor, className }: Props) {
    ...
    const toggleBlock = (type: string) => {
        ...
        const newType = (currentType === targetType && targetType !== 'paragraph' && type !== 'heading1') ? 'paragraph' : targetType

        // For H1 specifically, if we are already H1 (heading level 1), revert to paragraph
        if (type === 'heading1' && currentType === 'heading' && (currentBlock.props as any).level === 1) {
