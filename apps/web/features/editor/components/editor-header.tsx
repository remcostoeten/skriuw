...
function isValidCoverImageUrl(url?: string): boolean {
	...
	if (!url.startsWith('http://') && !url.startsWith('https://') && !url.startsWith('data:') && !url.startsWith('asset://')) return false
	const blockedDomains = ['example.com', 'example.org', 'example.net', 'placeholder.com', 'placehold.it', 'via.placeholder.com']
	try {
		const parsed = new URL(url)
		if (blockedDomains.some(domain => parsed.hostname === domain || parsed.hostname.endsWith(`.${domain}`))) return false
	} catch {
		return false
	}
	return true
											...
											<div className="space-y-2">
												<input
													...
													className={cn(
														...
														pasteUrlError ? "border-destructive" : "border-border"
													)}
													autoFocus
												/>
												{pasteUrlError && (
