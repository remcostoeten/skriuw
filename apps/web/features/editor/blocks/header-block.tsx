...
const CATEGORIES = [
    { id: 'personal', label: 'Personal', color: 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20' },
    { id: 'work', label: 'Work', color: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' },
...
export const headerBlockSpec = createReactBlockSpec(
    ...
    },
    {
        render: ({ block, editor, contentRef }) => {
            ...
            const currentCategory = CATEGORIES.find(c => c.id === block.props.category) || CATEGORIES[0]
                                ...
                                <div className="absolute top-full left-0 mt-2 w-40 z-50 rounded-lg border bg-popover p-1 shadow-md animate-in fade-in zoom-in-95 duration-100">
                                    {CATEGORIES.map((cat) => (
                                        ...
                                        >
                                            ...
                                            {cat.label}
                                            {block.props.category === cat.id && <Check className="ml-auto w-3 h-3" />}
                                        </button>
                                    ))}
                                </div>
                            )}
