...
export const shadcnTableBlockSpec = createReactBlockSpec(
    {
        type: 'shadcnTable',
        propSchema: {
            // Storing 2D grid as a simplified JSON string to avoid schema complexity
            tableData: {
                        ...
                        <TableHeader>
                            <TableRow>
                                {data[0]?.map((cell, colIndex) => (
                                    <TableHead key={`header-${colIndex}`} className="relative group/col min-w-[100px]">
                            ...
                            {data.slice(1).map((row, rowIndex) => (
                                <TableRow key={`row-${rowIndex + 1}`}>
                                    {row.map((cell, colIndex) => (
                                        <TableCell key={`cell-${rowIndex + 1}-${colIndex}`} className="p-2 relative group/cell">
                                            <Input
                                                value={cell}
