				...
				{isEditing ? (
					<div className='flex items-center gap-1 flex-1'>
						<input
							...
							value={editing.value}
							onChange={function (e) {
								setEditing({ id: node.id, value: e.target.value })
							}}
					...
					<>
						...
						<div className='opacity-0 group-hover/item:opacity-100 flex items-center gap-1 transition-opacity'>
							...
							<button
								onClick={function () {
									setEditing({ id: node.id, value: node.name })
								}}
								className='p-1 hover:bg-muted rounded transition-colors'
								title='Rename'
							>
...
export const fileTreeBlockSpec: any = createReactBlockSpec(
	...
	},
	{
		render: function ({ block, editor }) {
			...
			const [nodes, setNodes] = useState<TNode[]>(function () {
				return parseTreeString(content)
			})
