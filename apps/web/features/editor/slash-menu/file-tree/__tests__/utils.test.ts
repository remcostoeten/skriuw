...
describe('File Tree Utils', () => {
	...
	describe('buildTreeFromFiles', () => {
		...
		it('should sort files and folders correctly', () => {
			...
			const files: TFile[] = [
				...
				{ path: 'folder/c.ts', content: 'content' }
			]
	...
	describe('Node Operations', () => {
		...
		it('should add child node', () => {
			...
			const allFiles = flattenTreeToFiles(updatedTree)
			expect(allFiles.find((f) => f.path === "src/utils.ts")).toBeDefined()
		})
...
`
		...
		it('should handle round-trip serialization and parsing', () => {
			const originalFiles: TFile[] = [
				{ path: 'src/components/Button.tsx', content: 'content' },
				{ path: 'src/utils/index.ts', content: 'content' },
				{ path: 'package.json', content: '{}' },
