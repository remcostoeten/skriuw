...
export function ConfigModal({ isOpen, onClose, component, onSave }: ConfigModalProps) {
	...
	const handleNodeUpdate = useCallback(
		(id: string, updates: Partial<TNode>) => {
		...
		input.onchange = async (e) => {
			...
			try {
				...
				if (!data.files.every((f: unknown) => typeof (f as TFile).path === 'string')) {
					console.error(
						'Invalid file entries in JSON: each entry must have a path string'
					)
					return
				}
...
function TreeEditorNode({
	...
	return (
		<div
			...
			aria-expanded={node.type === 'folder' ? (node.isExpanded ?? false) : undefined}
		>
			<div
				className={cn(
