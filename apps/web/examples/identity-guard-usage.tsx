		...
		<div className='p-6 max-w-2xl mx-auto space-y-4'>
			...
			<div className='bg-muted p-4 rounded-lg'>
				...
				<ul className='text-sm space-y-1 text-muted-foreground'>
					<li>
						• If you have a session (authenticated or anonymous), operations proceed
						normally
					</li>
			...
			{result && (
				<div
					className={`p-3 rounded ${result.startsWith('✅') ? 'bg-green-100 text-green-800' : result.startsWith('🔐') ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
				>
					{result}
				</div>
			)}
			...
			<div className='text-xs text-muted-foreground space-y-1'>
				<p>
					<strong>Try this:</strong> If you're not signed in, click any button to see the
					auth modal open automatically.
				</p>
				<p>
					<strong>Developer notes:</strong> Check the browser console for operation logs.
				</p>
			</div>
		</div>
	)
