...
function journalEntryTitle(entry: JournalEntry): string {
	...
	return entry.content.trim().slice(0, 35) || "Empty entry";
}

function JournalStatsLoading() {
	return (
		<div className="space-y-4 p-3" aria-hidden="true">
			<div className="grid grid-cols-2 gap-2">
				{Array.from({ length: 4 }, (_, index) => (
					<div key={index} className="border border-border/60 bg-card/40 p-3">
						<div className="h-px w-8 bg-foreground/[0.1]" />
						<div className="mt-3 h-px w-16 bg-foreground/[0.07]" />
					</div>
				))}
			</div>
			<div className="space-y-2">
				<div className="h-px w-24 bg-foreground/[0.08]" />
				<div className="grid grid-cols-10 gap-1">
					{Array.from({ length: 30 }, (_, index) => (
						<div key={index} className="aspect-square bg-foreground/[0.06]" />
					))}
				</div>
			</div>
		</div>
...
export function JournalSidebar({ selectedDate, onSelectDate, className }: JournalSidebarProps) {
	...
	const startRename = (entry: JournalEntry) => {
		setRenamingId(entry.id);
		setRenameDraft(entry.title ?? "");
	};

	const commitRename = (entry: JournalEntry) => {
		const next = renameDraft.trim();
		if ((entry.title ?? "") !== next) {
			updateEntry.mutate({ id: entry.id, title: next || null });
		}
		setRenamingId(null);
	};
							...
							{calendarDays.map((day) => {
								...
								return (
									...
									>
										{format(day, "d")}
										{hasEntry && !isSelected && (
											<span className="absolute bottom-0.5 left-1/2 h-0.5 w-0.5 -translate-x-1/2 rounded-full bg-status-planned" />
										)}
										{hasEntry && isSelected && (
									...
									>
										<div className="flex min-w-0 flex-1 flex-col gap-0.5">
											<div className="flex items-center gap-2">
												...
												{mood && (
													<span className={cn("text-[11px]", mood.color)}>
														{mood.icon}
													</span>
												)}
											</div>
