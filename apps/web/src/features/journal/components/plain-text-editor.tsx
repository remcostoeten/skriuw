...
export function PlainTextEditor({
	...
	const insertTag = useCallback(
		(tagName: string) => {
			...
			const replacement = replaceActiveTagMention(
				content,
				tagCursorStart,
				cursorPos,
				tagName,
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
		...
		if (e.key === "ArrowDown") {
			...
			setSelectedSuggestionIdx(
				(prev) => (prev - 1 + popupOptions.length) % popupOptions.length,
			);
	...
	return (
		<div className="relative">
			<span id={helpTextId} className="sr-only">
				Type at-sign to open tag suggestions. Use arrow keys to move through results, Enter
				or Tab to insert a tag, and Escape to close the menu.
			</span>
							...
							{popupOptions.map((option, idx) => (
								<button
									...
									className={cn(
										...
										option.kind === "create" &&
											"border-t border-border text-status-planned",
									)}
								>
									{option.kind === "create" ? (
