...
export function JournalIcsImportDialog({ open, onOpenChange, entries }: Props) {
	...
	async function handleFile(file: File) {
		...
		try {
			...
			setPhase({ step: "preview", fileName: file.name, text, parsed, plan });
		} catch (error) {
			const message =
	...
	function replan(nextMode: JournalImportMode) {
		...
		setPhase({
			...phase,
			error: undefined,
			plan: planJournalIcsImport(phase.parsed, entries, nextMode),
		});
	}
	...
	async function handleConfirm() {
		...
		if (backend.mode === "server") {
			const preview = phase;
			setPhase({ step: "importing", done: 0, total: 1 });
			try {
				const response = await fetch(`/api/workspace/journal/import?mode=${mode}`, {
					method: "POST",
				...
				if (!response.ok || !payload.summary) {
					throw new Error(payload.error || "Calendar import failed.");
				}
				await queryClient.invalidateQueries({ queryKey: journalKeys.entries() });
				setPhase({ step: "done", summary: payload.summary, failures: [] });
				showUserToast(summarizeJournalImport(payload.summary), "success");
			} catch (error) {
				const message = error instanceof Error ? error.message : "Calendar import failed.";
				setPhase({ ...preview, error: message });
		...
		const plan = planJournalIcsImport(
			{ ...phase.parsed, totalEvents: events.length, events, skipped: [] },
			entries,
			mode,
		);
					...
					<div className="flex flex-col gap-3">
						{phase.error && (
							<p role="alert" className="text-xs text-destructive">
								{phase.error} No entries were changed.
							</p>
						)}
