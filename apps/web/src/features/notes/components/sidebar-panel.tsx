...
export function SidebarPanel({
  ...
  const visibleSearchFolders = searchResults.folders.slice(
    0,
    maxSearchResultsPerType,
  );
  const totalVisibleSearchResults =
    visibleSearchFiles.length + visibleSearchFolders.length;
  ...
  const closeSearch = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      const activeTreeItem =
        sidebarPanelRef.current?.querySelector<HTMLElement>(
  ...
  const handleSectionReorder = useCallback(
    (draggedId: string, targetId: string) => {
      ...
      const orderedSections = [...sidebarStore.config.sections].toSorted(
        (left, right) => {
          if (left.type === "file-tree" && right.type !== "file-tree")
            return -1;
  ...
  const getSectionMoveProps = useCallback(
    (sectionId: string) => {
      ...
      return {
        ...
        canMoveDown:
          currentIndex !== -1 && currentIndex < movableIds.length - 1,
      };
    },
    [sidebarStore],
  );
            ...
            <div className="flex items-center gap-2 md:gap-2.5 w-full justify-between">
                ...
                <SidebarConfigManager
                  ...
                  onToggleSectionVisibility={
                    sidebarStore.toggleSectionVisibility
                  }
