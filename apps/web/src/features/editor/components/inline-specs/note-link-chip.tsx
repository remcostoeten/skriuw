...
export function NoteLinkChip({ title }: Props) {
	const { activeFileId, resolver } = useNoteLinkContext();
	const { openNote, openNoteInNewTab, createAndOpenNote, isCreatingTitle } = useNoteLinkActions();
	...
	function handleClick(event: React.MouseEvent<HTMLButtonElement>) {
		...
		if (isResolved && resolved.targetNoteId) {
			if (openInNewTab) {
				openNoteInNewTab(resolved.targetNoteId);
			} else {
