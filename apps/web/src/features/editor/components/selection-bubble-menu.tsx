...
export function CustomPromptWidget({
  onSubmit,
  onClose,
}: TCustomPromptWidgetProps) {
...
function getToolbarItems(toolbar: HTMLElement): HTMLElement[] {
  return Array.from(
    toolbar.querySelectorAll<HTMLElement>(FOCUSABLE_ITEMS),
  ).filter((el) => !el.closest(".skriuw-fmt-dropdown"));
}
...
export function SelectionBubbleMenu({
  ...
  return (
    ...
    >
      <span
        ref={focusRingRef}
        className="skriuw-fmt-focus-ring"
        aria-hidden="true"
      />
      <BlockTypeMenu
        editor={editor}
      ...
      />
      {onAddComment ? (
        <CommentPopover editor={editor} onAddComment={onAddComment} />
      ) : null}
      {onAiSpellCheck ||
      onAiContinueWriting ||
      onAiAction ||
