...
So the code block needs its own background — currently it's transparent, showing the `--card` surface underneath. For the redesign, give it its own surface (e.g. `--muted` / `--theme-bg-hover`).

For the redesign: use `--theme-bg-editor` / `--card` as the editor surface, `--muted` / `--theme-bg-hover` for code block background, `--muted-foreground` for secondary UI (language label, line numbers), `--border` for outlines, and `--ring` for focus states.
