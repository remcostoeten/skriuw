...
import {
  blockToPlainText,
  blocksToMarkdown,
} from "@/features/editor/lib/editor-serialization";
...
export function useAiEditorHandle({
  editor,
  onEditorReady,
  wrapperRef,
}: Params) {
    ...
    onEditorReady({
      ...
      beginStreamingContinue: () =>
        createStreamingApplier({
          ...
          placeFirst: (blocks) => {
            ...
            if (
              anchor &&
              (anchorType === "paragraph" || anchorType === "quote")
            ) {
      ...
      deleteBlocks: (ids) => {
        const docIds = new Set(
          editor.document.map((b: { id: string }) => b.id),
        );
      ...
      replaceContent: (markdown) => {
        ...
        const corrected =
          parsed.length > beforeTexts.length
            ? selectCorrectedIndices(beforeTexts, parsedTexts).map(
                (k) => parsed[k],
              )
            : parsed;
        // biome-ignore lint/suspicious/noExplicitAny: schema-shaped blocks
