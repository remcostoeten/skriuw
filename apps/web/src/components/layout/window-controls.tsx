...
export function WindowControls() {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        const unlistenMaximized = await appWindow.onMaximized(() => setIsMaximized(true))
        const unlistenUnmaximized = await appWindow.onUnmaximized(() => setIsMaximized(false))
        
        cleanup.push(unlistenMaximized, unlistenUnmaximized)
      } catch (error) {
  ...
  return (
    <div className="flex items-center h-full" data-tauri-drag-region="false">
      ...
      >
        {isMaximized ? (
          <Square className="w-3 h-3 group-hover:scale-110 transition-transform" strokeWidth={2.5} />
        ) : (
