...
export class TauriFilesystemAdapter implements StorageAdapter {
	...
	capabilities: StorageAdapterCapabilities = { backends: ['filesystem'], syncMode: 'local-only' }

	async create<T>(storageKey: string, data: any, options?: CreateAdapterOptions): Promise<T> {
		const invoke = await getInvoke()
		const payload = await invoke<string>('tauri_fs_storage_create', {
			storageKey,
	...
	async readOne<T>(
		...
		const invoke = await getInvoke()
		const payload = await invoke<string | null>('tauri_fs_storage_read_one', {
			storageKey,
			id,
			userId: options?.userId ?? null
		})
	...
	async readMany<T>(storageKey: string, options?: BatchReadAdapterOptions): Promise<T[]> {
		const invoke = await getInvoke()
		const payloads = await invoke<string[]>('tauri_fs_storage_read_many', {
			storageKey,
			userId: options?.userId ?? null
		})
	...
	async update<T>(
		...
		const invoke = await getInvoke()
		const payload = await invoke<string | null>('tauri_fs_storage_update', {
			storageKey,
			id,
			data: JSON.stringify(data),
			userId: options?.userId ?? null
	...
	async delete(storageKey: string, id: string, options?: DeleteAdapterOptions): Promise<boolean> {
		const invoke = await getInvoke()
		return invoke<boolean>('tauri_fs_storage_delete', {
			storageKey,
			id,
			userId: options?.userId ?? null
		})
