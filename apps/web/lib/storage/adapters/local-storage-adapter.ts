...
export class LocalStorageAdapter implements StorageAdapter {
	...
	async read<T>(storageKey: string, options?: ReadAdapterOptions): Promise<T[] | T | undefined> {
		...
		if (options?.getById) {
			...
			return (location ? location.list[location.index] : undefined) as unknown as
				| T
				| undefined
		}
	...
	async update<T>(
		...
		const isMoving =
			data.parentFolderId !== undefined && data.parentFolderId !== currentItem.parentFolderId
		...
		for (const { id, data } of updates) {
			...
			if (location) {
				...
				const isMoving =
					...
					data.parentFolderId !== currentItem.parentFolderId
