...
export const db = {
    async create<T>(table: string, data: any): Promise<T> {
        ...
        const [created] = await getDatabase().insert(t).values(serialize(data, jsonFields)).returning()
        return deserialize(created, jsonFields) as T
    },
    ...
    async upsert<T>(table: string, data: any): Promise<T> {
        ...
        const [result] = await getDatabase()
            ...
            .returning()
        return deserialize(result, jsonFields) as T
    },

    async findById<T>(table: string, id: string): Promise<T | null> {
        const { table: t, jsonFields } = getConfig(table)
        const [result] = await getDatabase().select().from(t).where(eq(t.id, id))
        return result ? deserialize(result, jsonFields) as T : null
    },

    async findAll<T>(table: string): Promise<T[]> {
        const { table: t, jsonFields } = getConfig(table)
        const results = await getDatabase().select().from(t)
        return results.map(r => deserialize(r, jsonFields) as T)
    },
    ...
    async update<T>(table: string, id: string, data: any): Promise<T | null> {
        ...
        const [updated] = await getDatabase()
            ...
            .where(eq(t.id, id))
            .returning()
        return updated ? deserialize(updated, jsonFields) as T : null
    },
    ...
    async delete(table: string, id: string): Promise<boolean> {
        ...
        const [deleted] = await getDatabase().delete(t).where(eq(t.id, id)).returning()
        return !!deleted
    },
    ...
    async deleteMany(table: string, ids: string[]): Promise<number> {
        ...
        const { table: t } = getConfig(table)
        const deleted = await getDatabase().delete(t).where(inArray(t.id, ids)).returning()
    ...
    async createMany<T>(table: string, items: any[]): Promise<T[]> {
        ...
        const created = await getDatabase().insert(t).values(serialized).returning()
        return created.map(r => deserialize(r, jsonFields) as T)
    },
