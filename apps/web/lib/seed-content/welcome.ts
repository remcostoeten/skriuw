...
export function getWelcomeContent(): any[] {
	...
	return [
		...
		code(`// You're not reading docs. You're inside the editor.
// Press / anywhere to see what else you can create.
const idea = "just start typing"`, 'typescript'),
		...
		p([
			{ text: 'Click the boxes. They persist. Use these to think through anything.' },
		]),
		...
		bullet([{ text: 'Ctrl+P', styles: { code: true } }, { text: ' — search everything instantly' }]),
		bullet([{ text: '/', styles: { code: true } }, { text: ' — insert any block type' }]),
		bullet([{ text: '[[', styles: { code: true } }, { text: ' — link to another note' }]),
		bullet([{ text: 'Ctrl+\\', styles: { code: true } }, { text: ' — split view, two notes side by side' }]),

		empty(),

		p([
			{ text: 'No account required. Your notes live locally. ', styles: { italic: true } },
		]),
		p([
			{ text: 'Sign in only if you want them to persist across devices.' },
		]),

		empty(),

		p([
			{ text: 'Now delete this note and start writing.', styles: { bold: true } },
		]),
	]
}
