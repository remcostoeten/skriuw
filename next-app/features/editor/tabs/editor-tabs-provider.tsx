...
export function EditorTabsProvider({ children }: EditorTabsProviderProps) {
  ...
  const [activeNoteId, setActiveNoteId] = useState<string | null>(() => readPersistedState().activeNoteId)
  ...
  const openTab = useCallback(
    (tab: EditorTabInput, options: { activate?: boolean } = { activate: true }) => {
      ...
      setTabs(prev => {
        const nextTitle = tab.title?.trim() || prev.find(t => t.noteId === tab.noteId)?.title || 'Untitled'
  ...
  const closeOtherTabs = useCallback((noteId: string) => {
  ...
  const pruneTabs = useCallback((validIds: Set<string>) => {
    if (!validIds.size) {
      return
    }
    setTabs(prev => {
