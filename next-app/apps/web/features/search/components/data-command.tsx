...
export const DataCommand: FC<{
	...
	onClose?: () => void
	defaultPath?: string[]
}> = memo(({ items, onClose, defaultPath = [] }) => {
	const listRef = useRef<ComponentRef<typeof CommandList> | null>(null)
	const [commandChainKeys, setCommandChainKeys] = useState<string[]>(defaultPath)
	...
	const appendCommandHistory = ({ key, value }: { key: string; value: CommandHistoryItem }) => {
		fetching.current.set(key, !!value.isLoading)
		setCommandHistory((prev) => ({ ...prev, [key]: value }))
	}

	const commandKeyToItemMap = useRef(new Map<string, CommandDataItem | undefined>())
	...
	const addComandItemsToMap = (chainKey: string, commandItems: CommandDataItem[]) =>
		commandItems.map((item) => addCommandItemToMap([chainKey, item.value].join('.'), item))
		...
		() =>
			commandChainKeys.reduce(
				(acc, curr, idx, arr) => {
					...
					const currentItem =
						acc.list.find((i) => i?.value === curr) || commandKeyToItemMap.current.get(chainKey)
					...
					const key = isLastItem ? [chainKey, search].filter(Boolean).join(':') : chainKey
	...
	return (
		<Command
			...
			onKeyDown={(e) => {
				const currentFocusName = listRef.current
					?.querySelector('[aria-selected="true"]')
					?.getAttribute('data-value')
				const currentFocus = refinedChain.list.find((i) => i?.value === currentFocusName)
		...
		>
				...
				<div className="flex items-center gap-1 px-3 py-1.5 text-xs text-muted-foreground border-b">
					{refinedChain.chain.map((i, idx) => (
						<Fragment key={i.value}>
							...
							{idx < refinedChain.chain.length - 1 && <ChevronRight className="size-3" />}
						</Fragment>
					))}
				</div>
			)}
			<CommandInput
				...
				placeholder={currentContext?.searchPlaceholder ?? 'Search...'}
			/>
					...
					(item) =>
						item && (
							...
							>
								...
								{item.loadItems && <ChevronRight className="ml-2 size-4 text-muted-foreground" />}
							</MemoizedCommandItem>
						)
				)}
			</CommandList>
