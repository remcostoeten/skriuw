...
case "$mode" in
  ...
  -h|--help|help)
    cat <<'EOF'
Usage: ./tools/scripts/build.sh [check|browser|desktop|workspace|ci]
...
EOF
  ...
  *)
    ...
    printf 'Run ./tools/scripts/build.sh --help for usage.\n' >&2
    exit 2
    ;;
esac
...
case "$mode" in
  check) total_steps=18 ;;
  ci) total_steps=20 ;;
  ci:rust) total_steps=4 ;;
  ci:desktop) total_steps=1 ;;
  ci:renderer) total_steps=13 ;;
...
if gate_includes renderer; then
  run_step "Build entrypoint contract" "build-entrypoints" "$repo_dir/tools/scripts/test-build.sh"
fi
if gate_includes renderer || gate_includes release; then
  run_step "Browser SQLite WASM module" "browser-wasm" "$repo_dir/tools/scripts/build-browser-wasm.sh"
fi
if gate_includes rust; then
...
if gate_includes renderer; then
  run_step "Test layout" "test-layout" "$repo_dir/tools/scripts/check-test-layout.sh"
  ...
  print_metric "$(vitest_summary "$last_log")"
  run_step "Storybook shell installer and props suite" "storybook-shell-tests" bun --cwd="$repo_dir/packages/storybook-shell" run verify
  print_metric "$(vitest_summary "$last_log")"
