...
func DetectProjectStructure(rootDir string) (*ProjectStructure, error) {
	...
	if isMonorepo {
		...
		for _, dir := range workspaceDirs {
			...
			discoveredTools := discoverToolsFromDir(rootDir, dir, pm)
...
func discoverToolsFromDir(rootDir, relDir string, pm string) []DiscoveredTool {
	fullPath := filepath.Join(rootDir, relDir)
	pkgPath := filepath.Join(fullPath, "package.json")
	...
	// Only discover tools in tools/ directory
	if !strings.Contains(relDir, "tools/") {
		return nil
	}

	// Check for common tool scripts
	toolScripts := []string{"seed", "seed-cli", "seed-quick", "seeder"}
	tools := []DiscoveredTool{}

	if pkg.Scripts != nil {
		for _, scriptName := range toolScripts {
			if _, hasScript := pkg.Scripts[scriptName]; hasScript {
				key := filepath.Base(relDir)
				tool := DiscoveredTool{
					Key:     key + "-" + scriptName,
					Name:    getAppDisplayName(pkg) + " - " + scriptName,
					Dir:     relDir,
					Command: []string{pm, "run", scriptName},
					Package: pkg,
				}
