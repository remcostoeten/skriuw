...
GOOS=${OS} GOARCH=${ARCH} go build -ldflags="-s -w" -o bin/servo-${OS}-${ARCH} .
if [ "$OS" = "linux" ]; then
...
if [ "$OS" != "linux" ] || [ "$ARCH" != "aarch64" ]; then
    echo "  → Linux (arm64)..."
    GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o bin/servo-linux-arm64 .
fi

# macOS (amd64)
if [ "$OS" != "darwin" ] || [ "$ARCH" != "x86_64" ]; then
    echo "  → macOS (amd64)..."
    GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o bin/servo-darwin-amd64 .
fi
...
# Windows (arm64)
echo "  → Windows (arm64)..."
GOOS=windows GOARCH=arm64 go build -ldflags="-s -w" -o bin/servo-windows-arm64.exe .
