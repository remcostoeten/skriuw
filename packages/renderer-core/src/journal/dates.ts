...
/** True when `value` is a `YYYY-MM-DD` key naming a real calendar day. */
...
/** The local-time `DateKey` for `date`. */
...
/** The `DateKey` for today on this device's clock. */
export function todayKey(): DateKey {
  return dateKeyOf(new Date());
}

/** Local midnight on `key`'s day; an `Invalid Date` for a malformed key. */
...
/** The key `offset` days after `key` (negative moves back), crossing months and years. */
...
/** The month `offset` months after `current`. */
...
/** True when both keys name the same year and month. */
export function sameMonth(left: MonthKey, right: MonthKey): boolean {
  return left.year === right.year && left.month === right.month;
}

/** The `YYYY-MM` prefix every `DateKey` in `month` starts with. */
...
/** Monday-first two-letter weekday column labels for the month grid. */
