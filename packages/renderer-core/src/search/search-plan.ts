/**
 * Turns a query string into a runnable search plan: parsed filters, the note
 * ids they allow, and whether full-text search is needed at all.
 */

import type { SearchHit } from "../contracts/workspace";
import { referenceKey } from "../references/types";
...
/** Parses `query` and resolves its filters against `state` into a plan a search surface can run. */
