...
export function detectErrorCode(message: string): CrudErrorCode {
    ...
    if (lowerMessage.includes('duplicate') || lowerMessage.includes('unique')) return CrudErrorCode.ALREADY_EXISTS
    if (lowerMessage.includes('constraint') || lowerMessage.includes('foreign key')) return CrudErrorCode.CONSTRAINT_VIOLATION
