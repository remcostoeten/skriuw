...
describe('Error Code Detection', () => {
	describe('detectErrorCode', () => {
		...
		it('should detect CONSTRAINT_VIOLATION errors', () => {
			expect(detectErrorCode('Foreign key constraint failed')).toBe(
				CrudErrorCode.CONSTRAINT_VIOLATION
			)
			expect(detectErrorCode('Constraint violation on insert')).toBe(
				CrudErrorCode.CONSTRAINT_VIOLATION
			)
		})
		...
		it('should detect PERMISSION_DENIED errors', () => {
			...
			expect(detectErrorCode('Access denied for resource')).toBe(
