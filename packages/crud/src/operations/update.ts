/**
 * @fileoverview Update operation
 * @module @skriuw/crud/operations/update
 */
        ...
        if (options?.optimistic && options?.previousValue) {
            ...
            queueMicrotask(async () => {
                try {
                    ...
                    if (actual) {
                        options.onOptimisticSettled?.(successResult(actual, startTime, { optimistic: true }), false)
                    } else {
                        options.onOptimisticSettled?.(errorResult<T>(createNotFoundError(storageKey, id), startTime), true)
                    }
                } catch (error) {
                    options.onOptimisticSettled?.(errorResult<T>(createCrudError(error), startTime), true)
                }
            })
