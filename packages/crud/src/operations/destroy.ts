				...
				try {
					if (options.cascade) {
						await handleCascadeDelete(
							options.cascade.relations,
							options.cascade.foreignKey,
							id
						)
					}
					...
					options.onOptimisticSettled?.(
						successResult(true, startTime, { optimistic: true }),
						false
					)
				} catch (error) {
					options.onOptimisticSettled?.(
						errorResult<boolean>(createCrudError(error), startTime),
						true
					)
				}
			})
					...
					if (!continueOnError) {
						return {
							...
							summary: {
								...
								succeeded,
								failed,
								skipped: ids.length - i - batch.length
							},
							meta: {
								timestamp: startTime,
...
async function handleCascadeDelete(
	...
	for (const relationKey of relations) {
		try {
			const related = await getAdapter().read<BaseEntity & Record<string, unknown>>(
				relationKey,
				{ getAll: true }
			)
...
async function handleRecursiveDelete(
	...
	if (depth >= MAX_RECURSIVE_DEPTH) {
		console.warn(
			`Recursive delete max depth (${MAX_RECURSIVE_DEPTH}) reached for ${storageKey}:${parentId}`
		)
		return
	}

	try {
		const all = await getAdapter().read<BaseEntity & Record<string, unknown>>(storageKey, {
			getAll: true
		})
