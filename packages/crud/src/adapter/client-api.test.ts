/**
 * @fileoverview Client API Adapter Integration Tests
 * Tests the adapter that will be used by the web app to communicate with API routes
 * 
 * These tests verify:
 * 1. Storage key to endpoint mapping works correctly
 * 2. HTTP methods are called correctly
 * 3. Response parsing works as expected
...
describe('Client API Adapter', () => {
    ...
    describe('Mock Fetch Adapter', () => {
        ...
        it('should throw on non-JSON responses', async () => {
            ...
            await expect(adapter.read('Skriuw_notes', { getAll: true }))
