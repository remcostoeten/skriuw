    ...
    switch (noteName) {
        case 'Sync Seeds Workflow':
            return JSON.stringify([
                ...
                paragraph('This guide explains how Skriuw maintains consistent seed data for both Guest and Authenticated users.'),
                paragraph(''),
                ...
                paragraph('This led to "drift" where guests would see different content than signed-up users.'),
                paragraph(''),
                heading(2, 'The Solution'),
                paragraph('We now use a **Unified Seed System**. The `sync-seeds` script acts as the bridge:'),
                codeBlock('bun sync-seeds', 'bash'),
                paragraph('This script performs the following actions:'),
                numberedItem('Reads the Single Source of Truth from `packages/db/scripts/sync-seeds.ts`'),
                ...
                paragraph(''),
                paragraph('Now, every new user account will be initialized with exactly these notes! 🚀'),
            ]);
