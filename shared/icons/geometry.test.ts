...
import type { AnimatedPart, Clip } from "./geometry";
import { ICON_MOTIONS, type Track } from "./motion";
import type { AnimatedIconId } from "./parts";
import { containsPoint, flattenPath, type Point } from "./path";
import { isIdentityPose, resolvePose, transformPoint, type ResolvedPose } from "./sample";
...
test("the visible parts of every animated icon redraw its static glyph exactly at rest", () => {
  for (const icon of ICONS) {
    ...
    assert.equal(wrong.length, 0, `${icon}: ${wrong.length} sample points differ, first at ${wrong[0]}`);
  }
});
...
test("every animated part id matches exactly one part of its icon", () => {
  for (const icon of ICONS) {
    ...
    for (const part of ANIMATED_GEOMETRY[icon].parts) {
      if (part.within) assert.ok(ids.includes(part.within), `${icon}.${part.id}: within unknown part`);
      if (part.occluder) assert.ok(ids.includes(part.occluder.follows), `${icon}.${part.id}: occluder follows unknown part`);
    }
  }
});
...
test("parts that are not in the glyph stay invisible at the start and end of the motion", () => {
  for (const icon of ICONS) {
    for (const part of ANIMATED_GEOMETRY[icon].parts) {
      ...
      const ends = track ? [track.keyframes[0]!, track.keyframes[track.keyframes.length - 1]!] : [{}];
      const ink = partShape(part);
      ...
      for (const end of ends) {
        ...
        if (part.hidden === "opacity") {
          ...
          assert.equal(seen.length, 0, `${icon}.${part.id}: shows through its window at an end of its motion`);
        } else {
          const occluder = shape(part.occluder!.d);
          const seen = points.filter((point) => !occluder(transformPoint(pose, origin, point)));
          assert.equal(seen.length, 0, `${icon}.${part.id}: not covered by its occluder at an end of its motion`);
        }
      }
    }
  }
