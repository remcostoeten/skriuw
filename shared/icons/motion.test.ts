...
function hiddenParts(icon: AnimatedIconId): Set<string> {
  return new Set(ANIMATED_GEOMETRY[icon].parts.filter((part) => part.hidden).map((part) => part.id));
}
...
test("keyframes are ordered, start at 0 and end at 1", () => {
  for (const icon of ICONS) {
    for (const track of ICON_MOTIONS[icon].tracks) {
      ...
      assert.equal(offsets[offsets.length - 1], 1, `${icon}.${track.part}: last keyframe is not at 1`);
      for (let index = 1; index < offsets.length; index += 1) {
        assert.ok(offsets[index]! >= offsets[index - 1]!, `${icon}.${track.part}: keyframes out of order`);
      }
...
test("every visible part starts at rest and ends at rest or on a declared symmetry", () => {
  for (const icon of ICONS) {
    ...
    for (const track of motion.tracks) {
      ...
      assert.ok(isIdentityPose({ ...rest, rotate: 0 }), `${icon}.${track.part}: ends away from rest`);
      assert.ok(
  ...
  for (const icon of ICONS) {
    ...
    for (const track of ICON_MOTIONS[icon].tracks) {
      ...
      for (const keyframe of track.keyframes) {
        assert.equal(resolvePose(keyframe).opacity, 1, `${icon}.${track.part}: animates opacity at ${keyframe.at}`);
      }
    }
  }
});
...
test("sampling a track honours its delay, keyframes and per-segment easing", () => {
  ...
  const shoulders = ICON_MOTIONS.account.tracks.find((candidate) => candidate.part === "shoulders")!;
