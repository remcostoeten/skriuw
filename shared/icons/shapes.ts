...
function roundedRect(x: number, y: number, width: number, height: number, radius: number, clockwise: boolean): string {
...
export function outlineRect(x: number, y: number, width: number, height: number, radius: number, weight = 1.5): string {
  return (
    roundedRect(x, y, width, height, radius, true) +
    roundedRect(x + weight, y + weight, width - 2 * weight, height - 2 * weight, Math.max(0, radius - weight), false)
  );
}
