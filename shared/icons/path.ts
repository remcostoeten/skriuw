...
type Segment =
  ...
  | { kind: "A"; rx: number; ry: number; rotation: number; large: boolean; sweep: boolean; to: Point }
  | { kind: "Z" };
...
function tokenize(d: string): string[] {
  ...
  while (index < d.length) {
    ...
    if (!number || number.index !== index) throw new Error(`Malformed number at ${index} in "${d}"`);
  ...
  while (index < tokens.length) {
    ...
    switch (lower) {
      ...
      case "c": {
        ...
        segments.push({ kind: "C", c1: point(args[0]!, args[1]!), c2, to: point(args[4]!, args[5]!) });
        current = point(args[4]!, args[5]!);
        reflectCubic = c2;
        break;
      }
      case "s": {
        const c1: Point = lastCubic ? [2 * current[0] - lastCubic[0], 2 * current[1] - lastCubic[1]] : current;
      ...
      case "t": {
        const c: Point = lastQuad ? [2 * current[0] - lastQuad[0], 2 * current[1] - lastQuad[1]] : current;
...
function arcToCenter(from: Point, segment: Extract<Segment, { kind: "A" }>) {
  ...
  const factor = (segment.large === segment.sweep ? -1 : 1) * Math.sqrt(Math.max(0, numerator / denominator));
    ...
    switch (segment.kind) {
      ...
      case "C": {
        ...
        for (let step = 1; step <= CURVE_STEPS; step += 1) {
          ...
          const u = 1 - t;
          polygon.push([
            u * u * u * x0 + 3 * u * u * t * segment.c1[0] + 3 * u * t * t * segment.c2[0] + t * t * t * segment.to[0],
            u * u * u * y0 + 3 * u * u * t * segment.c1[1] + 3 * u * t * t * segment.c2[1] + t * t * t * segment.to[1],
          ]);
        }
        current = segment.to;
        break;
      ...
      case "A": {
        ...
        for (let step = 1; step <= CURVE_STEPS; step += 1) {
          ...
          polygon.push([arc.cos * px - arc.sin * py + arc.cx, arc.sin * px + arc.cos * py + arc.cy]);
        }
        current = segment.to;
        break;
      }
...
export function containsPoint(polygons: readonly Point[][], [x, y]: Point, rule: FillRule = "nonzero"): boolean {
  let winding = 0;
  let crossings = 0;
