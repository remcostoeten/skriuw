...
- `apps/instantdb` holds the desktop/web client; all feature work lives under `src/`.
...
- `bun run dev` starts the InstantDB app directly on port 42069; use when debugging Next.js-specific issues.
- `bun run build` produces a Next.js production build; pair with `bun run tauri:build` inside `apps/instantdb` for desktop bundles.
