...
| `AttachImage` / replicated | Available owner note, unique image ID, verified content hash and bytes | Identical ID/hash/owner is **No-op**; divergent identity is conflict | Different images commute once chunks exist | Note purge removes metadata only after conflict/checkpoint references release it; blob deletion follows reference proof | Apply after the asset bytes are downloaded, digest-verified, and durably stored; missing or corrupt asset content fails the pull before anything applies | remote attach convergence; duplicate identical attach; note tombstone blocks attach; missing chunk; corrupt chunk |
