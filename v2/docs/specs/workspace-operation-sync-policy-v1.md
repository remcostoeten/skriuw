...
| `AttachImage` | Unsupported v1 | Image metadata cannot be made applicable before the content-addressed blob is verified and available. Idempotency ultimately keys on IDs and content hash. | Asset deletion/retention must account for every document, cover, journal, checkpoint, and device reference. | Metadata is small, but the media bytes require chunk transport; the whole operation is blocked in v1. | Returns `unsupported_operation`; the local asset and blocked state must be retained and shown. |
