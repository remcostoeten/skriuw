...
- [x] Third-party importers. Previewed, atomic local import ships for Markdown, plain text, Simplenote (`notes.json`), Bear (TextBundle and `.bear2bk`), Obsidian, Notion Markdown/database CSV, and Apple Notes Markdown. Folder, individual-file, ZIP, and provider-archive intake is bounded against traversal, symlinks, duplicate paths, excessive depth/count, and expanded size. Provider timestamps, typed properties, tags, internal links, and supported local images migrate when representable; complex frontmatter stays lossless raw source, raw-note tags fall back to a typed property, ambiguous assets stay text, and every unreadable or unsupported file appears in preview and completion reports. See [the import guide](docs/provider-import.md), [spec](docs/specs/provider-import.md), and [ADR-0024](docs/adr/0024-previewed-atomic-provider-import.md).
