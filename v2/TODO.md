...
- [ ] Third-party importers. Adapter framework shipped (`app/src/import/`): one detect/parse contract feeding the safety-aware plan/commit pipeline, with Markdown, plain-text, Simplenote (`notes.json`), and Bear (textbundle) sources. Remaining: Obsidian (frontmatter → properties, `![[embeds]]`, attachment folders), Notion zip, Apple Notes (needs an export-format decision), tag import for sources that carry tags.
