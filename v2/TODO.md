...
Bounded correctness model: `spikes/ui-architecture/src/editors/bounded-correctness.ts` now covers a pure movable window, scroll-anchor adjustment, focus/selection restoration, canonical edit reconciliation, and rejection of edits outside the rendered window. Five regressions pass. It is not wired to the ProseMirror DOM candidate; window recycling, browser selection, clipboard/find, IME, undo, accessibility traversal, plugins, and native presentation remain open.
