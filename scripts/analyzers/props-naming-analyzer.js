...
class PropsNamingAnalyzer {
				...
				for (const item of items) {
					...
					try {
						...
						if (stat.isDirectory()) {
							...
							if (this.options.scope && !fullPath.includes(this.options.scope))
								continue
							files.push(fullPath)
						}
					} catch (e) {}
		...
		try {
			...
			for (const issue of sortedIssues) {
				...
				if (issue.type === 'wrong_name') {
					...
					newLine = line.replace(/interface\s+Props\s*\{/, 'type Props = {')
				}
		...
		for (const [backupPath, info] of entries) {
			try {
				if (fs.existsSync(backupPath)) {
					...
					console.log(
						`  ${c('green', '✅')} Reverted: ${path.relative(this.projectRoot, info.originalPath)}`
					)
					revertedCount++
				}
			} catch (e) {
				console.error(
					`  ${c('red', '❌')} Failed to revert ${info.originalPath}: ${e.message}`
				)
			}
		}
		this.saveManifest({})
	...
	async fixIssues() {
		...
		for (const [filePath, fileIssues] of Object.entries(issuesByFile)) {
			console.log(
				`${c('cyan', '📄')} ${path.relative(this.projectRoot, filePath)} (${fileIssues.length} issues)`
			)
		...
		if (dryRun) {
			console.log(
				`${c('cyan', '📊')} Would fix ${c('bold', totalFixed)} Props naming issues.`
			)
	...
	generateReport() {
		...
		console.log(
			`Issues found: ${this.issueCount > 0 ? c('yellow', this.issueCount) : c('green', '0')}`
		)
		console.log('')

		if (this.issues.length === 0) {
			console.log(
				`${c('green', '✅')} No Props naming issues found! Your code follows the convention.`
			)
			return
		}
		...
		if (wrongNameIssues.length > 0) {
			console.log(
				`${c('red', '🔴')} Single non-exported type not named "Props": ${wrongNameIssues.length}`
			)
			wrongNameIssues.slice(0, 5).forEach((issue) => {
				console.log(`   ${c('cyan', issue.file)}:${issue.line}`)
				console.log(
					`   ${c('dim', 'Current:')} ${issue.currentName} → ${c('green', 'Props')}`
				)
			})
			if (wrongNameIssues.length > 5)
				console.log(`   ${c('dim', `... and ${wrongNameIssues.length - 5} more`)}`)
			console.log('')
		}
		...
		if (interfaceIssues.length > 0) {
			...
			if (interfaceIssues.length > 3)
				console.log(`   ${c('dim', `... and ${interfaceIssues.length - 3} more`)}`)
			console.log('')
		}

		if (exportedPropsIssues.length > 0) {
			console.log(
				`${c('magenta', '⚠️')}  Exported type named "Props" (needs manual fix): ${exportedPropsIssues.length}`
			)
		...
		console.log('══════════════')
		console.log(
			`${c('green', '✅')} type Props = { ... }           ${c('dim', '// Single non-exported type')}`
		)
		console.log(`${c('red', '❌')} type ComponentProps = { ... }  ${c('dim', '// Wrong name')}`)
		console.log(
			`${c('red', '❌')} interface Props { ... }        ${c('dim', '// Should be type')}`
		)
		console.log(
			`${c('red', '❌')} export type Props = { ... }    ${c('dim', '// Exported should have specific name')}`
		)
	}
}
...
`)
	...
	switch (choice.trim()) {
		...
		case '5': {
			...
			console.log('')
			await new PropsNamingAnalyzer({
				scope: scopePath.trim() || undefined
			}).analyzeDirectory(webAppDir)
			break
		}
		case '0':
...
async function main() {
	...
	console.log(
		`${c('cyan', '📁')} Target directory: ${path.relative(analyzer.projectRoot, targetDir) || '.'}`
	)
