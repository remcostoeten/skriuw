...
import { useEditorSetting } from "@/shared/data/settings";
...
export function WordWrapExample() {
  const { value: wordWrap, setValue: setWordWrap } = useEditorSetting<boolean>('wordWrap');
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Label htmlFor="word-wrap-switch">
          Word Wrap (via useEditorSetting): {wordWrap ? 'Enabled' : 'Disabled'}
        </Label>
      </div>
      ...
      <div className="p-4 border rounded-md">
        <p className="text-sm text-muted-foreground mb-2">
          Editor word wrap is currently: <strong>{hasWordWrap ? 'ENABLED' : 'DISABLED'}</strong>
        </p>
        <p className="text-xs">
          This affects how text flows in the BlockNote editor.
          When enabled, long lines will wrap to fit the editor width.
        </p>
      </div>
...
export function ConditionalWordWrapExample() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h3 className="text-lg font-semibold">Conditional Rendering Example</h3>
      ...
      {hasWordWrap ? (
        ...
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
          <p className="text-yellow-800">
            ⚠️ Word wrap is disabled. Text will stay on single lines.
          </p>
        </div>
      )}

      <div className="text-sm text-muted-foreground">
        This shows how you can conditionally render UI based on user preferences.
      </div>
    </div>
  );
}