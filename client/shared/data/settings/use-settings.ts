...
import { DEFAULT_EDITOR_SETTINGS, EDITOR_SETTINGS, validateEditorSetting } from "./editor-settings";
import type { UserSetting, SettingsConfig } from "../types";

/**
 * Hook for accessing and managing user settings
 */
export function useSettings<T extends Record<string, any> = Record<string, any>>() {
  const { settings, updateSetting, resetSettings, isLoading } = useSettingsContext();

  const getSetting = useCallback(<K extends keyof T>(key: K): T[K] => {
    return settings[key as string] as T[K];
  }, [settings]);

  const setSetting = useCallback(<K extends keyof T>(key: K, value: T[K]) => {
    ...
    try {
      ...
      Object.entries(importedSettings).forEach(([key, value]) => {
        ...
        if (validation === true) {
          ...
          console.warn(`Skipping invalid setting ${key}:`, validation);
        }
      });
    } catch (error) {
...
export function useEditorSetting<T = any>(key: string) {
  ...
  if (!setting) {
    throw new Error(`Unknown editor setting: ${key}`);
  }
...
export function useTypedSettings<T extends Record<string, any>>(
  ...
  const getTypedSetting = useCallback(<K extends keyof T>(key: K): T[K] => {
  ...
  const setTypedSetting = useCallback(<K extends keyof T>(key: K, value: T[K]) => {
    const config = schema[key];
