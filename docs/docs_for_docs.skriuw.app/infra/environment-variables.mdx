...
[Self-host with Docker](./self-host-docker.mdx) for the custom-domain rebuild.
...
| `SEMANTIC_SEARCH_PROVIDER`     | Optional | Embedding provider: `google` (default) or `ollama`.                                                                |
| `SEMANTIC_SEARCH_OLLAMA_URL`   | Optional | Ollama URL used for semantic embeddings. Defaults to `http://127.0.0.1:11434`.                                     |
| `SEMANTIC_SEARCH_OLLAMA_MODEL` | Optional | Ollama embedding model. Defaults to `embeddinggemma`; use any installed embedding-capable model.                   |
...
Collaboration runs on a Cloudflare Worker in `party/`, not the Docker stack.
...
[Collaboration setup](./collaboration-setup.mdx).
