...
- **Vim mode** — one setting (also `mod+alt+i` or “Toggle Vim mode” in the palette) makes both editors modal. The rendered editor gets a Vim layer built for the block document: normal, insert, visual, and visual-line modes, counts, operators with motions and text objects, `f`/`t` and `/` search, registers including the system clipboard, dot repeat, macros, and `:w`, `:q`, `:N`, `:noh`, and `:s`. The raw Markdown view runs CodeMirror’s Vim with the same `:` commands, where `:q` returns to the rendered editor; plain `j` and `k` move by wrapped screen rows there too. See [ADR-0042](adr/0042-modal-vim-editing.md).
...
- **AI writing actions** — with AI enabled, an **Ask AI** button sits in the editor pane, the selection toolbar gains a named **Ask AI** entry, and the command palette gains one per action. All three open the same anchored menu over the text it is about — never a modal, so the paragraph you are judging the result against stays on screen. Selections can be rewritten, improved, spell- and grammar-checked, shortened, lengthened, simplified, retoned, translated, turned into a diagram, or handed a custom instruction; the whole note can be continued, summarised, outlined, titled, mined for tasks, or given tag suggestions. Note-wide actions are reachable with nothing selected, and selection actions stay listed with the reason they are waiting. Actions that ask you nothing start on the click that chose them; the four that take an instruction get a second pane in the same popover, where a disclosure shows the exact text that will leave the device and which model receives it. Every run then reports itself in the note: labelled steps for sending, waiting on the model, and writing, a placeholder where the text will land, elapsed time, a note when a stream has gone quiet, and a **Stop** that ends the run the moment you press it rather than when the provider gets around to agreeing. A failure names the step it failed in and the one thing you can do about it — add a key, pick another model, send less — with Retry alongside. Output streams into a preview, never into your note, so stopping, failing, or discarding leaves the note byte-for-byte unchanged; accepting applies as a single undoable edit that you can replace the selection with or insert below, and you can copy or retry instead. **Diagram** draws a Mermaid flowchart, sequence, state, class, or ER diagram from the selection and inserts it below the text instead of replacing it; a reply that would not render is sent back to the model once with the renderer's error before you see it. Extracted tasks and suggested tags arrive as a reviewable list you tick through in the same card before anything is added, and then become ordinary tasks and tag chips. Oversized selections are refused with their actual size rather than quietly truncated, progress is announced for screen readers, everything is reachable by keyboard, and none of it exists until AI is enabled. See [ADR-0036](adr/0036-ai-results-are-reviewed-in-place.md) and [ADR-0040](adr/0040-ai-runs-are-visible-work.md).
- **Voice dictation** — with AI enabled, the palette's **AI: Dictate into note** opens a recording surface with a live level meter and clock. Stopping sends the recording to the speech-to-text model you pick (Groq Whisper or Gemini, with your own key, chosen per the same disclosure and consent rules as every remote provider) and the transcript comes back as raw text or, via the cleaned-up and structured modes, reformatted by your default AI model using shipped prompts you can customise. Nothing is written until you insert it — at the cursor or below, as one undoable edit — and if formatting fails the raw transcript is offered so dictated words are never lost. The recording lives only in memory for the one request; it never touches your notes, disk, sync, or history.
