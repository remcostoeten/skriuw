# ADR-0049: One shared icon system, Fluent Regular, motion as data
...
Lucide drawings into `app/src/shared/icons/static.tsx` and normalised their
...
- **Fluent Regular is the only icon set.** `shared/icons` (`@skriuw/icons`)
...
- **Geometry and motion are data; platforms only draw.** Each animated
  ...
  (`shared/icons/parts.ts`), resolved by the generator into
...
- **Desktop keeps named exports.** `static.tsx` is a table of named
  ...
  shared with mobile (`shared/icons/registry.ts`).
...
- Adding an icon means adding its Fluent name to `catalog.ts` and running
  `bun shared/icons/generate.ts`; adding an animation means a part split in
