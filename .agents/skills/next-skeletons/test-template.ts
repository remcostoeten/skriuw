...
function printReport(report: RouteReport) {
	...
	if (missingInSkeleton.length > 0) {
		console.log(
			`Elements missing from skeleton: ${missingInSkeleton.map((c) => c.selector).join(", ")}`,
		);
	}
	if (missingInLoaded.length > 0) {
		console.log(
			`Elements in skeleton but not in loaded state: ${missingInLoaded.map((c) => c.selector).join(", ")}`,
		);
	}

	for (const v of violations) {
		console.log(
			`  SHIFT: ${v.selector} moved (${v.deltaX}px, ${v.deltaY}px), size delta (${v.deltaWidth}px, ${v.deltaHeight}px)`,
		);
	...
	if (report.diffPercentage >= config.diffThreshold && violations.length === 0) {
		...
		console.log(
			`    Fix: The fallback shell's layout structure may not match the loaded state. Check the relevant Suspense boundary's fallback.`,
		);
	}
