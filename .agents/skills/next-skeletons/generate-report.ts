...
function formatReport(reports: RouteReport[]): string {
	...
	for (const report of reports) {
		...
		for (const v of violations) {
			lines.push(
				`  SHIFT: ${v.selector} moved (${v.deltaX}px, ${v.deltaY}px), size changed (${v.deltaWidth}px, ${v.deltaHeight}px)`,
			);
	...
	if (failed > 0) {
		...
		lines.push(
			"1. Fix the Suspense fallback shells for failing routes using the suggestions above.",
		);
