...
export type IcsParseResult = {
	calendarName?: string;
	productId?: string;
	/** Stable identity for matching UIDs from the same external calendar. */
...
function calendarSourceId(productId?: string, calendarName?: string): string {
	const parts = [productId, calendarName]
		.map((part) => part?.trim().replace(/\s+/g, " "))
		.filter((part): part is string => Boolean(part));
	return `ics:${(parts.join("|") || "unknown").slice(0, 480)}`;
}
...
export function parseJournalIcs(text: string): IcsParseResult {
	...
	for (const line of lines) {
		...
		} else if (parsed.name === "PRODID") {
			productId = unescapeIcsText(parsed.value).trim() || undefined;
		}
	}
	...
	return {
		calendarName,
		productId,
		calendarSourceId: calendarSourceId(productId, calendarName),
		totalEvents,
		events,
		skipped,
	};
...
export function planJournalIcsImport(
	...
	const existingByCalendarIdentity = new Map(
		existingEntries
			.filter(
				(entry) =>
					entry.calendarSourceId === parsed.calendarSourceId && entry.calendarSourceUid,
			)
			.map((entry) => [entry.calendarSourceUid as string, entry]),
	);
	...
	for (const event of parsed.events) {
		...
		const identityMatch = event.uid ? existingByCalendarIdentity.get(event.uid) : undefined;
		const existingId =
			identityMatch?.id ??
		...
		if (
			identityMatch &&
			identityMatch.dateKey !== event.dateKey &&
			existingByDate.has(event.dateKey) &&
			existingByDate.get(event.dateKey) !== identityMatch.id
		) {
			skipped.push({
				summary: event.title,
				dateKey: event.dateKey,
				reason: "The event moved to a day that already has a journal entry",
			});
			continue;
		}
