...
describe("planJournalIcsImport", () => {
	...
	test("matches a moved external event by calendar identity and UID", () => {
		const parsed = parseJournalIcs(
			wrapCalendar([
				"BEGIN:VEVENT",
				"UID:external-42",
		...
		const existing = makeEntry({
			...
			calendarSourceUid: "external-42",
		});

		const plan = planJournalIcsImport(parsed, [existing], "update");

		expect(plan.creates).toHaveLength(0);
		expect(plan.updates).toEqual([
			{ targetId: "entry-1", event: expect.objectContaining({ dateKey: "2026-04-02" }) },
		]);
	});

	test("does not move an external event onto an occupied journal day", () => {
		const parsed = parseJournalIcs(
			wrapCalendar([
				"BEGIN:VEVENT",
				"UID:external-42",
		...
		const occupied = makeEntry({ id: "entry-2", dateKey: "2026-04-02" });

		const plan = planJournalIcsImport(parsed, [existing, occupied], "update");

		expect(plan.updates).toHaveLength(0);
		expect(plan.skipped[0]?.reason).toContain("already has a journal entry");
	});
});
