...
Disconnecting desktop sync or choosing a credential-inclusive reset deletes the
secure credential. If the OS credential store is locked or unavailable, desktop
does not connect or fall back to plaintext storage.
...
- **Options page** (`src/options/App.tsx`): "Test & save" calls
  `verifyRawToken(apiBase, token)` _before_ writing anything to
