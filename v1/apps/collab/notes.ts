...
import { verifyCollabToken } from "../web/src/features/collaboration/lib/collab-token";
...
//
// Runs on YOUR Cloudflare account (deploy with `bun run party:deploy`), not the
// shared partykit.dev zone — so the 10k-custom-domain limit never applies.
//
// Set the secret with: `bunx wrangler secret put COLLAB_AUTH_SECRET -c apps/collab/wrangler.jsonc`
