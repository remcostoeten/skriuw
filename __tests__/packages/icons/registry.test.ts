...
import { GLYPH_NAMES } from "../../../packages/icons/catalog";
import { ANIMATED_GEOMETRY } from "../../../packages/icons/generated/animated";
import { GLYPHS } from "../../../packages/icons/generated/glyphs";
import { ICON_NAMES, ICON_REGISTRY } from "../../../packages/icons/registry";
import { selectGlyph } from "../../../packages/icons/select";
...
test("an animated registry entry animates the glyph it shows at rest", () => {
  for (const name of ICON_NAMES) {
    ...
    assert.equal(
      ANIMATED_GEOMETRY[entry.motion].glyph,
      entry.glyph,
      `${name}: motion is drawn on another glyph`,
    );
  }
});
...
test("every cataloged glyph has data and small sizes prefer the 20-unit drawing", () => {
  ...
  assert.equal(
    selectGlyph("text_whole_word", 32).grid,
    20,
    "falls back to the only drawing Fluent has",
  );
});
