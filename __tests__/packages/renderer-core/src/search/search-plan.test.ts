...
import {
  ...
  tag,
} from "../../../../apps/workspace/src/features/references/fixtures";
...
test("a filtered query limits after storage applies its candidate IDs", () => {
  ...
  const limit = 2;
  const plan = planWorkspaceSearch(state, "#design budget", limit);
  assert.equal(plan.fullTextLimit, limit);
  const hits = hitsFor([
    "note-plain",
    "note-person-only",
  ...
  assert.deepEqual(
    applySearchPlan(state, plan, hits, limit).map((hit) => hit.noteId),
    ["note-tag-only", "note-both"],
  );
});
