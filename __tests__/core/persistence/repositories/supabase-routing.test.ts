...
const notesPersistencePromise = import("@/core/persistence/notes");
const foldersPersistencePromise = import("@/core/persistence/folders");
const journalPersistencePromise = import("@/core/persistence/journal");
...
describe("repository remote routing", () => {
  test("notesRepository.list prefers Supabase when remote persistence is available", async () => {
    const notesPersistence = await notesPersistencePromise;
    ...
    const notes = await notesPersistence.listNotes();
  ...
  test("foldersRepository.destroy soft-deletes descendant folders and notes in Supabase mode", async () => {
    const foldersPersistence = await foldersPersistencePromise;
    ...
    await foldersPersistence.deleteFolder("folder-root" as never);
  ...
  test("journalRepository.destroyTag removes tag references before soft-deleting the tag in Supabase mode", async () => {
    const journalPersistence = await journalPersistencePromise;
    ...
    await journalPersistence.deleteJournalTag("tag-1" as never);
