...
test("groups follow definition order, none is empty, and each row appears once", () => {
  const groups = shortcutHelpGroups({ overrides: {}, platform: "linux" });
  const names = groups.map((group) => group.group);
  const definitionOrder = [...new Set(SHORTCUT_DEFINITIONS.map((entry) => entry.group))];
  assert.deepEqual(
    names,
    definitionOrder.filter((group) => names.includes(group)),
  );
  assert.ok(groups.every((group) => group.rows.length > 0));
  const ids = groups.flatMap((group) => group.rows.map((row) => row.id));
  assert.equal(new Set(ids).size, ids.length);
  assert.equal(shortcutHelpRowCount(groups), ids.length);
  assert.ok(ids.includes("createNote"));
  assert.ok(ids.includes("goToDocumentStart"));
});
