...
import type { NoteLockState, WorkspaceNode } from "@skriuw/renderer-core/contracts/workspace";
...
test("locked node ids list only the nodes carrying a lock timestamp", () => {
  const state = {
    sourceNodes: new Map([
      ["locked", { id: "locked", lockedAt: 10 } as WorkspaceNode],
      ["open", { id: "open", lockedAt: null } as WorkspaceNode],
      ["legacy", { id: "legacy" } as WorkspaceNode],
    ]),
  } as RendererState;
  assert.deepEqual(lockedNodeIds(state), ["locked"]);
});
