...
import { documentSaveOperations } from "@/features/editor/task-linking";
...
test("only the first promotion in a batch carries the document, once, from the unsaved revision", () => {
  const document = productSchema.node("doc", null, [
    productSchema.node("check_list", null, [
      productSchema.node("check_item", { checked: false, taskId: "task-1", blockId: "block-1" }, [
