...
} from "@/features/editor/code-highlight";
...
test("a registered language produces highlight tokens inside the highlighted text", () => {
  ...
  const jsTokens = highlightCode("js", code);
  assert.ok(jsTokens.length > 0);
