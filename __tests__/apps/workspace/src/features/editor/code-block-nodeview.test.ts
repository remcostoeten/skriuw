...
} from "@/features/editor/code-block-nodeview";
import type { MermaidRenderResult } from "@/features/editor/mermaid-render";
import { CODE_LANGUAGES, resolveHighlightLanguage } from "@/features/editor/code-highlight";
...
test("typing a letter jumps to the next matching language", () => {
  const { trigger, menu } = mountCodeBlock("");
  keydown(trigger, "ArrowDown");
  keydown(menu, "j");
