...
import { savedSearches } from "@/features/search/saved-searches";
...
test("saved queries preserve relationship syntax through serialization", () => {
  ...
  assert.deepEqual(savedSearches(JSON.parse(JSON.stringify(settings))), queries);
  assert.deepEqual(savedSearches(fixtureSettings()), []);
});

test("saved search preference rejects malformed and unbounded input", () => {
  for (const invalid of [null, "query", [42], [" "], ["x".repeat(513)], Array(101).fill("query")]) {
    assert.throws(() => savedSearches({ ...fixtureSettings(), savedSearches: invalid }), /invalid/);
  }
});
