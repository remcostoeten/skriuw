...
import {
  ...
  moodTrend,
  moodTrendSummary,
} from "@/features/journal/mood-trend";
import type { JournalEntry, MoodLevel } from "@/features/journal/model";
...
test("moodTrend spans the window oldest first and keeps gaps visible", () => {
  const trend = moodTrend(
    [entry("2026-09-18", "good"), entry("2026-09-16", null)],
    "2026-09-18",
    4,
  );
  assert.deepEqual(
...
test("entries outside the window do not count", () => {
  const trend = moodTrend(
    [entry("2026-08-01", "rough"), entry("2026-09-19", "great")],
    "2026-09-18",
    30,
  );
...
test("direction compares the two halves once each holds two rated days", () => {
  const up = moodTrend(
    [
      entry("2026-09-11", "low"),
      entry("2026-09-12", "rough"),
      entry("2026-09-17", "good"),
      entry("2026-09-18", "great"),
    ],
    "2026-09-18",
    8,
  );
  ...
  const down = moodTrend(
    [
      entry("2026-09-11", "great"),
      entry("2026-09-12", "good"),
      entry("2026-09-17", "low"),
      entry("2026-09-18", "rough"),
    ],
    "2026-09-18",
    8,
  );
  ...
  const steady = moodTrend(
    [
      entry("2026-09-11", "good"),
      entry("2026-09-12", "good"),
      entry("2026-09-17", "good"),
      entry("2026-09-18", "good"),
    ],
    "2026-09-18",
    8,
  );
  ...
  const sparse = moodTrend(
    [entry("2026-09-11", "rough"), entry("2026-09-18", "great")],
    "2026-09-18",
    8,
  );
