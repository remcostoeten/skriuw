...
import { BUILT_IN_PROMPTS } from "@/features/ai/prompts/built-in-prompts";
import {
  AI_EDITOR_ACTIONS,
...
test("extraction actions are the only ones producing a reviewable plan", () => {
  assert.deepEqual(
    AI_EDITOR_ACTIONS.filter(
      (candidate) => candidate.outcome === "tasks" || candidate.outcome === "tags",
    ).map((candidate) => [candidate.id, candidate.outcome]),
    [
      ["extract-tasks", "tasks"],
      ["suggest-tags", "tags"],
    ],
