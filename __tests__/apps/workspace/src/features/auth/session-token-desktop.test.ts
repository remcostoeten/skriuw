...
test("a sign-in during a pending credential load survives the load resolving", async () => {
  ...
  const sessionToken = await import("@/features/auth/session-token.ts?slow-load");
...
test("a sign-out during a pending credential load stays signed out", async () => {
  ...
  const sessionToken = await import("@/features/auth/session-token.ts?slow-clear");
...
test("a vault that refuses the credential keeps the in-memory session and reports it", async () => {
  ...
  const sessionToken = await import("@/features/auth/session-token.ts?no-keyring");
