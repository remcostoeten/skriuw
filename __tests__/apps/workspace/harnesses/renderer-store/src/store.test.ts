...
import {
  createInitialState,
  createRendererStore,
} from "../../../../../../apps/workspace/harnesses/renderer-store/src/store";
import type {
  RendererState,
  TreeProjection,
} from "../../../../../../apps/workspace/harnesses/renderer-store/src/types";
...
test("preserves canonical ordering and hidden active selection on collapse", () => {
  ...
  assert.deepEqual(store.getState().nodeOrder, [
    "folder",
    "note-child",
    "note-root",
    "note-disabled",
...
test("subscriber failures do not corrupt state or listener bookkeeping", () => {
  const store = storeWithDisabledNote();
  let healthyCalls = 0;
  const removeFailure = store.subscribe(
    (state) => state.activeNoteId,
    () => {
