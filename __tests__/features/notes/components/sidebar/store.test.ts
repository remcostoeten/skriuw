...
import type { Project, RecentItem, SidebarSection } from "@/features/notes/components/sidebar/types";
...
async function loadStoreModule() {
  ...
  return import(`@/features/notes/components/sidebar/store?test=${Math.random().toString(36).slice(2)}`);
}
...
describe("sidebar store workspace scoping", () => {
  test("keeps favorites, recents, custom sections, projects, and visibility prefs isolated per workspace", async () => {
    ...
    expect(
      useSidebarStore
        .getState()
        .config.sections.find((section: SidebarSection) => section.id === "search")?.isVisible,
    ).toBe(true);
    expect(
      useSidebarStore
        .getState()
    ...
    expect(useSidebarStore.getState().config.projects.map((project: Project) => project.name)).toEqual([
      "Workspace A Project",
    ]);
    expect(
      useSidebarStore
    ...
    expect(
      useSidebarStore
        .getState()
        .config.sections.find((section: SidebarSection) => section.id === "favorites")?.isVisible,
    ).toBe(false);
